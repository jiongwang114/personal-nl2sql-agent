...
"""
Unit tests for dataengineer/configuration/agent_config_loader.py
...
from dataengineer.configuration.agent_config_loader import (
...
class TestParseConfigPath:
    ...
    def test_explicit_non_existing_non_default_raises(self):
        with pytest.raises(DataEngineerException, match="not found"):
            parse_config_path("nonexistent_config.yml")

    def test_explicit_default_fallback(self, tmp_path, monkeypatch):
        """When config_file is 'conf/agent.yml' and doesn't exist anywhere, raises DataEngineerException."""
        # chdir to tmp_path (no conf/agent.yml there) and patch home to nonexistent path
        monkeypatch.chdir(tmp_path)
        with patch("dataengineer.configuration.agent_config_loader.Path.home", return_value=tmp_path / "noexist"):
    ...
    def test_home_config_found(self, tmp_path, monkeypatch):
        """Falls back to ~/.dataengineer/conf/agent.yml."""
        ...
        with patch("dataengineer.configuration.agent_config_loader.Path.home", return_value=tmp_path):
            result = parse_config_path("")
        assert result.name == "agent.yml"
...
class TestApplyProjectOverride:
    """_apply_project_override validates & merges ./.dataengineer/config.yml into agent_raw."""
    ...
    def test_no_override_is_noop(self):
        ...
        with patch(
            "dataengineer.configuration.agent_config_loader.load_project_override",
            return_value=None,
        ):
    ...
    def test_valid_default_datasource_flips_default_flags(self):
        ...
        uniformly across every entry point (REPL, dataengineer-api, SDK)."""
        agent_raw = self._base_raw()
        with patch(
            "dataengineer.configuration.agent_config_loader.load_project_override",
    ...
    def test_invalid_default_datasource_raises(self):
        ...
        with patch(
            ...
            with pytest.raises(DataEngineerException) as exc:
                _apply_project_override(agent_raw)
...
class TestLoadNodeConfig:
    def test_with_model(self):
        ...
        data = {"model": "gpt-4o"}
        with patch("dataengineer.configuration.agent_config_loader.NodeType.type_input", return_value={}):
...
class TestLoadAgentConfigResolution:
    ...
    is populated for every entry point (REPL, dataengineer-api, dataengineer-gateway, SDK),
    ...
    def test_project_overlay_wins_over_base_default(self, tmp_path, reset_global_singletons):
        """``.dataengineer/config.yml::default_datasource`` overrides the base default flag."""
        cfg = self._write_base_yaml(
            tmp_path,
            {
