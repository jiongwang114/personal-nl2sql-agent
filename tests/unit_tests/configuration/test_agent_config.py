...
"""
Additional unit tests for dataengineer/configuration/agent_config.py
...
from dataengineer.configuration.agent_config import (
    AgentConfig,
    BenchmarkConfig,
    DbConfig,
    DocumentConfig,
...
class TestAgentConfigServiceSelectors:
    ...
    def test_resolve_semantic_adapter_requires_explicit_choice_for_multiple_entries(self, tmp_path):
        ...
        with pytest.raises(DataEngineerException, match="Multiple semantic layers are configured"):
    ...
    def test_default_scheduler_service_rejects_multiple_defaults(self, tmp_path):
        with pytest.raises(DataEngineerException, match="Multiple scheduler services are marked"):
            self._make(
                tmp_path,
                services={
                    "datasources": {},
    ...
    def test_get_scheduler_config_requires_explicit_choice_when_multiple_instances_exist(self, tmp_path):
        ...
        with pytest.raises(DataEngineerException, match="set `scheduler_service` on the scheduler node"):
    ...
    def test_init_scheduler_services_requires_declared_type(self, tmp_path):
        with pytest.raises(DataEngineerException, match="must declare a scheduler `type`"):
            self._make(
                tmp_path,
                services={
                    "datasources": {},
...
class TestNormalizeProjectName:
    ...
    def test_replaces_slashes(self):
        from dataengineer.configuration.agent_config import _normalize_project_name
    ...
    def test_long_path_truncated_with_md5(self):
        ...
        from dataengineer.configuration.agent_config import _PROJECT_NAME_MAX_LEN, _normalize_project_name
    ...
    def test_derived_name_passes_backend_segment_check(self):
        ...
        from dataengineer.configuration.agent_config import _normalize_project_name
        from dataengineer.storage.rdb.sqlite_backend import _safe_path_segment
...
class TestAgentConfigProjectLayout:
    ...
    def test_overlong_project_name_rejected(self, tmp_path):
        from dataengineer.configuration.agent_config import _PROJECT_NAME_MAX_LEN
...
class TestServicesConfigFromDict:
    ...
    def test_legacy_databases_key_is_rejected(self):
        ...
        with pytest.raises(DataEngineerException, match="services.databases has been renamed to services.datasources"):
...
class TestProviderConfigurationDispatch:
    ...
    def test_active_model_raises_when_nothing_is_configured(self, tmp_path):
        ...
        assert "dataengineer init" in exc_info.value.message
