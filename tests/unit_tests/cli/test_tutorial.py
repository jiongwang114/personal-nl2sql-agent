...
import dataengineer.storage.metric.metric_init as metric_init
import dataengineer.storage.reference_sql.reference_sql_init as reference_sql_init
from dataengineer.cli import tutorial as tutorial_module
from dataengineer.cli.tutorial import BenchmarkTutorial, dict_to_yaml_str
...
class TestBenchmarkTutorialEnsureFiles:
    ...
    def test_creates_benchmark_directory_if_not_exists(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "dataengineer.cli.interactive_init.copy_data_file",
            mock_copy_data_file,
        )
...
class TestBenchmarkTutorialRun:
    ...
    def test_run_completes_successfully(self, tmp_path, monkeypatch):
        ...
        mock_copy_data_file = MagicMock()
        monkeypatch.setattr("dataengineer.cli.interactive_init.copy_data_file", mock_copy_data_file)

        mock_init_metadata = MagicMock()
        mock_init_sql = MagicMock()
        monkeypatch.setattr("dataengineer.cli.interactive_init.init_metadata_and_log_result", mock_init_metadata)
        monkeypatch.setattr("dataengineer.cli.interactive_init.overwrite_sql_and_log_result", mock_init_sql)
        ...
        assert result == 0
        output = buffer.getvalue()
        assert "Welcome to DataEngineer benchmark" in output
...
class TestBenchmarkTutorialInitMetrics:
    ...
    def test_init_metrics_success(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "dataengineer.storage.metric.metric_init.init_success_story_metrics",
            mock_init_metrics,
        )

        # Mock create_vector_connection to avoid creating dataengineer_db_ in working directory
        monkeypatch.setattr("dataengineer.storage.backend_holder.create_vector_connection", MagicMock())
