...
"""
Unit tests for dataengineer/cli/print_mode.py
"""
...
def _make_runner(**overrides):
    ...
    with (
        patch("dataengineer.cli.print_mode.load_agent_config") as mock_cfg,
        patch("dataengineer.cli.print_mode.AtReferenceCompleter") as mock_completer,
    ):
...
class TestWritePayload:
    def test_writes_json_line(self):
        with (
            patch("dataengineer.cli.print_mode.load_agent_config") as mock_cfg,
            patch("dataengineer.cli.print_mode.AtReferenceCompleter"),
        ):
            mock_cfg.return_value = MagicMock(datasource_configs={})
            from dataengineer.cli.print_mode import PrintModeRunner
...
class TestResumeSessionId:
    def test_resume_sets_session_id(self):
        ...
        with (
            patch("dataengineer.cli.print_mode.create_interactive_node", return_value=mock_node),
            patch("dataengineer.cli.print_mode.create_node_input", return_value=MagicMock()),
            patch("dataengineer.models.session_manager.SessionManager", return_value=mock_session_mgr),
        ):
            runner.run()
    ...
    def test_resume_subagent_name_from_args(self):
        ...
        with (
            patch("dataengineer.cli.print_mode.create_interactive_node", return_value=mock_node) as mock_create_node,
            patch("dataengineer.cli.print_mode.create_node_input", return_value=MagicMock()),
            patch("dataengineer.models.session_manager.SessionManager", return_value=mock_session_mgr),
        ):
            runner.run()
...
class TestRunUsesFactory:
    def test_run_calls_factory(self):
        ...
        with (
            patch("dataengineer.cli.print_mode.create_interactive_node", return_value=mock_node) as mock_create_node,
            patch("dataengineer.cli.print_mode.create_node_input", return_value=mock_input) as mock_create_input,
        ):
            runner.run()
...
class TestRunProxyTools:
    def test_run_applies_proxy_tools_when_patterns_set(self):
        ...
        with (
            patch("dataengineer.cli.print_mode.create_interactive_node", return_value=mock_node),
            patch("dataengineer.cli.print_mode.create_node_input", return_value=MagicMock()),
            patch("dataengineer.tools.proxy.proxy_tool.apply_proxy_tools") as mock_apply,
        ):
            runner.run()
