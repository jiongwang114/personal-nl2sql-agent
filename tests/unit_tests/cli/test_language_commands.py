...
from dataengineer.cli.language_app import LanguageSelection
...
_PATCH_LOAD = "dataengineer.cli.language_commands.load_project_override"
_PATCH_SAVE = "dataengineer.cli.language_commands.save_project_override"
...
class TestDirectCodeWithProjectFlag:
    def test_saves_to_project(self, commands):
        ...
        with (
            ...
            patch(_PATCH_SAVE, return_value="/tmp/.dataengineer/config.yml") as mock_save,
        ):
...
class TestUnknownCode:
    def test_warns_but_proceeds(self, commands):
        cmds, cli = commands
        with patch(_PATCH_LOAD, return_value=None), patch(_PATCH_SAVE, return_value="/tmp/.dataengineer/config.yml"):
