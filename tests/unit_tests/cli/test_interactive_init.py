...
from dataengineer.cli.interactive_init import (
    InteractiveInit,
...
class TestInit:
    ...
    def test_llm_config_probe_success(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.dict("sys.modules", {"dataengineer.models.openai_model": mock_module}):
    ...
    def test_config_file_generation(self, monkeypatch):
        ...
        with tempfile.TemporaryDirectory() as tmpdir, monkeypatch.context() as isolated_cwd:
            ...
            from dataengineer.configuration.project_config import ProjectTarget
    ...
    def test_optional_component_init(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("dataengineer.cli.interactive_init.Confirm.ask") as mock_confirm:
            ...
            with (
                patch("dataengineer.cli.interactive_init.Confirm.ask") as mock_confirm,
                patch("dataengineer.cli.interactive_init.init_metadata_and_log_result") as mock_metadata,
            ):
            ...
            with (
                patch("dataengineer.cli.interactive_init.Confirm.ask") as mock_confirm,
                patch("dataengineer.cli.interactive_init.Prompt.ask") as mock_prompt,
                patch("dataengineer.cli.interactive_init.init_metadata_and_log_result") as mock_metadata,
                patch("dataengineer.cli.interactive_init.overwrite_sql_and_log_result") as mock_sql,
            ):
...
class TestConfigureWorkspace:
    def test_success_creates_directory(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("dataengineer.cli.interactive_init.Prompt.ask", return_value=workspace):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("dataengineer.cli.interactive_init.Prompt.ask", return_value=workspace):
                with patch("pathlib.Path.mkdir", side_effect=PermissionError("denied")):
                    with patch("dataengineer.cli.interactive_init.print_rich_exception") as mock_print_exc:
                        result = init._configure_workspace()
...
class TestDisplayMethods:
    ...
    def test_display_completion_smoke(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert "dataengineer" in init.console.file.getvalue().lower()
...
class TestConfigureLLM:
    def test_empty_api_key_returns_false(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch("dataengineer.cli.interactive_init.select_choice", return_value="openai"),
                patch("dataengineer.cli.interactive_init.getpass", return_value=""),
            ):
    ...
    def test_api_key_provider_writes_provider_level_credentials(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch("dataengineer.cli.interactive_init.select_choice", side_effect=["kimi", "kimi-k2.5"]),
                patch("dataengineer.cli.interactive_init.Prompt.ask", return_value="https://api.moonshot.cn/v1"),
                patch("dataengineer.cli.interactive_init.getpass", return_value="test-key"),
                patch.dict("sys.modules", {"dataengineer.models.kimi_model": mock_module}),
            ):
                result = init._configure_llm()
    ...
    def test_api_key_provider_probe_failure_drops_pending_target(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch("dataengineer.cli.interactive_init.select_choice", side_effect=["openai", "gpt-4.1"]),
                patch("dataengineer.cli.interactive_init.Prompt.ask", return_value="https://api.openai.com/v1"),
                patch("dataengineer.cli.interactive_init.getpass", return_value="test-key"),
                patch.dict("sys.modules", {"dataengineer.models.openai_model": mock_module}),
            ):
                result = init._configure_llm()
...
class TestOverwriteSqlAndLogResult:
    def test_exception_is_caught_and_printed(self):
        ...
        with (
            patch(
                "dataengineer.configuration.agent_config_loader.load_agent_config", side_effect=RuntimeError("config error")
            ),
            patch("dataengineer.cli.interactive_init.print_rich_exception") as mock_print_exc,
        ):
            overwrite_sql_and_log_result(
...
class TestConfigureLLMSubscriptionOAuth:
    ...
    def test_subscription_success_writes_provider_section(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch("dataengineer.cli.interactive_init.select_choice", return_value="claude_subscription"),
                patch("dataengineer.cli.interactive_init.configure_claude_subscription", return_value=helper_return),
            ):
    ...
    def test_oauth_helper_failure_returns_false(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch("dataengineer.cli.interactive_init.select_choice", return_value="codex"),
                patch("dataengineer.cli.interactive_init.configure_codex_oauth", return_value=None),
