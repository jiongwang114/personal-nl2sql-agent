...
These tests protect the invariants consumed by :class:`DataEngineerCLI`:
...
- the registry matches the handler map wired in ``DataEngineerCLI``

Registry integrity is a pure-data concern, so no ``DataEngineerCLI`` construction is
needed here.
"""

from __future__ import annotations

from dataengineer.cli.slash_registry import (
    GROUP_ORDER,
    GROUP_TITLES,
    SLASH_COMMANDS,
    all_tokens,
...
class TestHandlerMapAlignment:
    """Handlers are wired in :meth:`DataEngineerCLI._build_slash_handler_map`.
    ...
    — no DataEngineerCLI instance needed.
    """
    ...
    def test_handler_map_covers_every_canonical_name(self):
        ...
        source = inspect.getsource(DataEngineerCLI._build_slash_handler_map)
        for spec in SLASH_COMMANDS:
            needle = f'"{spec.name}"'
