...
"""
Unit tests for dataengineer/cli/plan_hooks.py — PlanModeHooks.
...
from dataengineer.cli.plan_hooks import PlanModeHooks, UserCancelledException
...
def _make_hooks(broker, session, auto_mode=False):
    with patch("dataengineer.tools.func_tool.plan_tools.SessionTodoStorage") as MockStorage:
...
class TestGetPlanTools:
    def test_get_plan_tools_returns_tools(self, broker, session):
        ...
        with patch("dataengineer.tools.func_tool.plan_tools.PlanTool") as MockPlanTool:
