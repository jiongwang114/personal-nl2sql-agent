...
"""
Unit tests for dataengineer/cli/sub_agent_commands.py — SubAgentCommands.
...
class TestDoUpdateAgent:
    def test_wizard_cancelled_prints_message(self, cmds):
        with patch("dataengineer.cli.sub_agent_commands.run_wizard", return_value=None):
            cmds._do_update_agent()
        calls = [str(c) for c in cmds.cli_instance.console.print.call_args_list]
        assert any("cancelled" in c for c in calls)

    def test_wizard_exception_prints_error(self, cmds):
        with patch("dataengineer.cli.sub_agent_commands.run_wizard", side_effect=RuntimeError("wizard boom")):
            cmds._do_update_agent()
        calls = [str(c) for c in cmds.cli_instance.console.print.call_args_list]
    ...
    def test_reserved_name_blocked(self, cmds):
        ...
        with patch("dataengineer.cli.sub_agent_commands.run_wizard", return_value=result_agent):
            cmds._do_update_agent()
        calls = [str(c) for c in cmds.cli_instance.console.print.call_args_list]
