...
"""Regression guards for the DataEngineer TUI layout.
...
from dataengineer.cli.tui.app import DataEngineerApp


def _build_app() -> DataEngineerApp:
    return DataEngineerApp(status_tokens_fn=lambda: [], dispatch_fn=lambda _: None)


class TestCompletionsMenuWired:
    def test_completions_menu_wraps_completions_menu_control(self):
        """DataEngineerApp inlines prompt_toolkit's ``CompletionsMenu`` layout — a
    ...
    def test_menu_sits_between_input_and_bottom_separator(self):
        ...
        # DataEngineerApp wraps the HSplit directly; grab the children list.
