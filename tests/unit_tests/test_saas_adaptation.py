...
from dataengineer.storage.ext_knowledge.ext_knowledge_init import (
    init_ext_knowledge,
    init_success_story_knowledge,
    init_success_story_knowledge_async,
)
from dataengineer.storage.metric.metric_init import init_success_story_metrics, init_success_story_metrics_async
from dataengineer.storage.reference_sql.reference_sql_init import init_reference_sql, init_reference_sql_async
...
def _init_vector_backend(tmp_path):
    ...
    from dataengineer.storage.registry import clear_storage_registry
    from dataengineer.utils.path_manager import DataEngineerPathManager, reset_path_manager, set_current_path_manager

    init_backends(data_dir=str(tmp_path))
    pm = DataEngineerPathManager(datus_home=tmp_path, project_name="saas_test", project_root=tmp_path)
...
class TestSyncWrapperBackwardCompatibility:
    ...
    def test_metrics_sync_returns_three_tuple(self):
        ...
        with patch(
            "dataengineer.storage.metric.metric_init.init_success_story_metrics_async",
...
class TestInitExtKnowledgeStringParam:
    ...
    def test_string_csv_path_accepted(self, tmp_path):
        ...
        from dataengineer.storage.embedding_models import get_db_embedding_model
        from dataengineer.storage.ext_knowledge.store import ExtKnowledgeStore
...
def _load_pydantic_v2_classes():
    ...
    from dataengineer.schemas.base import BaseInput, BaseResult, CommonData
    ...
    from dataengineer.schemas.node_models import ExecuteSQLResult, ReflectionResult
    from dataengineer.schemas.semantic_agentic_node_models import SemanticNodeInput
    from dataengineer.tools.db_tools.config import ConnectionConfig
...
class TestPydanticV2ConfigDict:
    ...
    def test_models_still_instantiate_correctly(self):
        ...
        from dataengineer.schemas.base import BaseInput, BaseResult, CommonData
        from dataengineer.schemas.node_models import ExecuteSQLResult
...
class TestDBManagerFactory:
    ...
    def test_set_db_manager_factory_importable(self):
        ...
        from dataengineer.tools.db_tools.db_manager import set_db_manager_factory
    ...
    def test_factory_called_when_set(self):
        ...
        from dataengineer.tools.db_tools.db_manager import DBManager, db_manager_instance, set_db_manager_factory
...
class TestAgentConfigSkipInitDirs:
    ...
    def _make_config(self, tmp_path, skip_init_dirs=False):
        ...
        from dataengineer.configuration.agent_config import AgentConfig, NodeConfig
    ...
    def test_skip_init_dirs_does_not_call_path_manager(self, tmp_path):
        ...
        with patch("dataengineer.utils.path_manager.get_path_manager") as mock_pm:
            config = self._make_config(tmp_path, skip_init_dirs=True)
    ...
    def test_skip_init_dirs_does_not_call_init_embedding_models(self, tmp_path):
        ...
        with patch("dataengineer.configuration.agent_config.init_embedding_models") as mock_init:
    ...
    def test_skip_init_dirs_does_not_call_init_backends(self, tmp_path):
        ...
        with patch("dataengineer.storage.backend_holder.init_backends") as mock_init:
            self._make_config(tmp_path, skip_init_dirs=True)
            mock_init.assert_not_called()
