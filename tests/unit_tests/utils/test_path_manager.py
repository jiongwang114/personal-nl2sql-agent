...
"""Unit tests for dataengineer/utils/path_manager.py — CI tier, zero external deps."""
...
class TestDataEngineerPathManagerInit:
    """Tests for DataEngineerPathManager.__init__."""

    def test_default_home_is_dot_dataengineer(self):
        pm = DataEngineerPathManager()
        assert pm.datus_home == Path.home() / ".datus"

    def test_custom_home_is_resolved(self, tmp_path):
        pm = DataEngineerPathManager(datus_home=str(tmp_path))
        assert pm.datus_home == tmp_path.resolve()

    def test_tilde_expansion(self):
        pm = DataEngineerPathManager(datus_home="~/.dataengineer_test")
        assert "~" not in str(pm.datus_home)

    def test_update_home(self, tmp_path):
        pm = DataEngineerPathManager()
        new_home = tmp_path / "new_dataengineer"
    ...
    def test_project_root_defaults_to_cwd(self, tmp_path, monkeypatch):
        ...
        pm = DataEngineerPathManager(datus_home=str(tmp_path / "home"))
    ...
    def test_knowledge_base_home_kwarg_no_longer_accepted(self, tmp_path):
        ...
        with pytest.raises(TypeError):
            DataEngineerPathManager(datus_home=str(tmp_path / "dataengineer"), knowledge_base_home="")


class TestDataEngineerPathManagerProperties:
    """Tests for DataEngineerPathManager directory properties."""
    ...
    def pm(self, tmp_path):
        ...
        return DataEngineerPathManager(
            datus_home=str(tmp_path / "dataengineer"),
            project_name="proj",
...
class TestDataEngineerPathManagerConfigPaths:
    """Tests for configuration file paths."""

    @pytest.fixture
    def pm(self, tmp_path):
        return DataEngineerPathManager(datus_home=str(tmp_path / "dataengineer"))
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("pid_file_path", [], ("run_dir", "dataengineer-agent-api.pid")),
...
class TestDataEngineerPathManagerDataPaths:
    ...
    def test_rag_storage_path_creates_dir(self, pm):
        ...
        assert path == pm.project_data_dir / "dataengineer_db"
        assert path.exists()
...
class TestResolveRunDir:
    """Tests for DataEngineerPathManager.resolve_run_dir."""

    def test_without_run_id(self, tmp_path):
        base = tmp_path / "base"
        path = DataEngineerPathManager.resolve_run_dir(base, "myns")
        assert path == base / "myns"
        assert path.exists()

    def test_with_run_id(self, tmp_path):
        base = tmp_path / "base"
        path = DataEngineerPathManager.resolve_run_dir(base, "myns", "20250101")
        assert path == base / "myns" / "20250101"
        assert path.exists()


class TestResolveConfigPath:
    """Tests for DataEngineerPathManager.resolve_config_path."""
...
class TestEnsureDirs:
    """Tests for DataEngineerPathManager.ensure_dirs."""
    ...
    def test_ensure_templates_creates_template_dir_and_copies_defaults(self, pm):
        with patch("dataengineer.utils.resource_utils.copy_data_file") as mock_copy:
            pm.ensure_templates()
...
class TestGetPathManager:
    ...
    def test_returns_instance(self):
        pm = get_path_manager()
        assert isinstance(pm, DataEngineerPathManager)
    ...
    def test_set_current_path_manager_accepts_path_manager_instance(self, tmp_path):
        current = DataEngineerPathManager(tmp_path / "tenant_home")
    ...
    def test_set_current_path_manager_accepts_agent_config(self, tmp_path):
        agent_config = SimpleNamespace(path_manager=DataEngineerPathManager(tmp_path / "agent_home"))
    ...
    def test_path_manager_argument_has_highest_precedence(self, tmp_path):
        explicit_pm = DataEngineerPathManager(tmp_path / "explicit_home")
        agent_config = SimpleNamespace(path_manager=DataEngineerPathManager(tmp_path / "agent_home"))
    ...
    def test_agent_config_has_precedence_over_explicit_home_and_context(self, tmp_path):
        agent_pm = DataEngineerPathManager(tmp_path / "agent_home")
...
class TestResetPathManager:
    ...
    def test_reset_clears_context_local_home(self, tmp_path):
        set_current_path_manager(tmp_path)
        reset_path_manager()
        from dataengineer.utils import path_manager
    ...
    def test_context_var_preserves_project_shard_round_trip(self, tmp_path):
        """Storing a DataEngineerPathManager via ContextVar preserves project_name sharding."""
        project_root = tmp_path / "proj"
        pm = DataEngineerPathManager(
            datus_home=str(tmp_path / "home"),
            project_name="-tmp-proj",
