...
"""Unit tests for dataengineer/utils/path_utils.py — CI tier, zero external deps."""
...
from dataengineer.utils.path_utils import (
...
class TestSafeRmtree:
    ...
    def test_oserror_during_delete_returns_false(self, tmp_path):
        ...
        target.mkdir()
        with patch("dataengineer.utils.path_utils.shutil.rmtree", side_effect=OSError("permission denied")):
            result = safe_rmtree(target, force=True)
        assert result is False
