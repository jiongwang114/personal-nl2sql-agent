...
"""
Unit tests for dataengineer/models/session_manager.py.
...
from dataengineer.models.session_manager import (
    SessionManager,
...
class TestSessionManagerPathManagerInjection:
    def test_uses_explicit_path_manager_when_session_dir_missing(self, tmp_path):
        path_manager = DataEngineerPathManager(tmp_path / "tenant_home", project_name="tenant")
...
class TestGetDetailedUsage:
    ...
    def test_invalid_scope_raises_datus_exception(self, tmp_path, bad_scope):
        """Scope values with special characters raise DataEngineerException."""
        with pytest.raises(DataEngineerException, match="Invalid scope"):
