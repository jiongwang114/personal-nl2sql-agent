...
class TestBuildEnhancedMessage:
    ...
    def test_basic_message(self):
        from dataengineer.agent.node.gen_sql_agentic_node import build_enhanced_message
...
class TestPrepareTemplateContext:
    ...
    def test_basic_context(self):
        from dataengineer.agent.node.gen_sql_agentic_node import prepare_template_context
...
class TestSetupToolPatternGenSQL:
    ...
    def test_wildcard_unknown_type_logs_warning(self, real_agent_config, mock_llm_create):
        node = _make_node_extra2(real_agent_config, mock_llm_create)
        with patch("dataengineer.agent.node.gen_sql_agentic_node.logger.warning") as mock_warning:
    ...
    def test_exception_in_setup_is_caught(self, real_agent_config, mock_llm_create):
        ...
        with (
            ...
            patch("dataengineer.agent.node.gen_sql_agentic_node.logger.error") as mock_error,
        ):
...
class TestExecuteStreamGenSQLError:
    ...
    async def test_execute_stream_error_yields_error_action(self, real_agent_config, mock_llm_create):
        ...
        from dataengineer.agent.node.gen_sql_agentic_node import GenSQLAgenticNode
        from dataengineer.schemas.action_history import ActionStatus
...
def _make_gensql_node(node_config=None, agent_config=None):
    ...
    from dataengineer.agent.node.agentic_node import AgenticNode
    from dataengineer.agent.node.gen_sql_agentic_node import GenSQLAgenticNode
    from dataengineer.cli.execution_state import InteractionBroker, InterruptController
...
class TestGenSQLNodeInput:
    ...
    def test_with_schemas(self):
        from dataengineer.schemas.node_models import TableSchema
...
class TestGenSQLSystemPromptCurrentDate:
    ...
    def test_system_prompt_uses_reference_date(self, real_agent_config, mock_llm_create):
        ...
        with patch(
            "dataengineer.utils.time_utils.get_default_current_date",
