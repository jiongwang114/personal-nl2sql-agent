...
from dataengineer.configuration.agent_config import DashboardConfig
...
_SKILLS_DIR = _REPO_ROOT / "dataengineer" / "resources" / "skills"
...
class TestGenDashboardExecuteStream:
    ...
    async def test_execute_stream_raises_without_input(self, real_agent_config, mock_llm_create):
        """execute_stream should raise DataEngineerException when self.input is None."""
        _add_dashboard_config(real_agent_config)
        with patch.dict(sys.modules, _BI_MODULES_PATCH):
            from dataengineer.agent.node.gen_dashboard_agentic_node import GenDashboardAgenticNode
...
class TestGenDashboardCustomNodeName:
    ...
    def test_alias_system_prompt_template_name(self, real_agent_config, mock_llm_create):
        ...
        with patch.dict(sys.modules, _BI_MODULES_PATCH):
            ...
            with patch("dataengineer.prompts.prompt_manager.get_prompt_manager") as mock_pm:
