...
from dataengineer.schemas.node_models import ExecuteSQLResult, GenerateSQLResult
from dataengineer.schemas.schema_linking_node_models import SchemaLinkingResult
...
class TestNodeInstantiation:
    ...
    def test_node_invalid_type_in_constructor(self):
        ...
        from dataengineer.agent.node.generate_sql_node import GenerateSQLNode
...
class TestNodeInitialize:
    ...
    def test_initialize_with_model_already_set_as_llm(self):
        ...
        with patch("dataengineer.agent.node.node.LLMBaseModel.create_model") as mock_create:
            node._initialize()
    ...
    def test_initialize_with_model_name_string(self):
        ...
        with patch("dataengineer.agent.node.node.LLMBaseModel.create_model", return_value=mock_llm) as mock_create:
            node._initialize()
    ...
    def test_initialize_from_agent_config_nodes(self):
        ...
        with patch("dataengineer.agent.node.node.LLMBaseModel.create_model", return_value=mock_llm):
            node._initialize()
            assert node.model == mock_llm
...
class TestNodeFromDict:
    ...
    @patch("dataengineer.agent.node.gen_table_agentic_node.DBFuncTool")
    @patch("dataengineer.models.base.LLMBaseModel.create_model")
