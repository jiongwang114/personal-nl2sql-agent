...
"""
Unit tests for dataengineer/agent/workflow_runner.py.
...
from dataengineer.schemas.base import BaseResult
...
class TestInitializeWorkflow:
    def test_initialize_workflow_creates_workflow(self):
        ...
        with patch("dataengineer.agent.workflow_runner.generate_workflow", return_value=mock_wf):
...
class TestResumeWorkflow:
    def test_resume_workflow_success(self):
        ...
        with patch("dataengineer.agent.workflow_runner.Workflow.load", return_value=mock_wf):
    ...
    def test_resume_workflow_load_failure_raises(self):
        ...
        with patch("dataengineer.agent.workflow_runner.Workflow.load", side_effect=FileNotFoundError("not found")):
...
class TestFinalizeWorkflow:
    ...
    def test_with_workflow_saves_and_returns_metadata(self, tmp_path):
        ...
        with patch("dataengineer.agent.workflow_runner.get_trace_url", return_value=None):
    ...
    def test_trace_url_stored_in_metadata(self, tmp_path):
        ...
        with patch("dataengineer.agent.workflow_runner.get_trace_url", return_value="http://trace.url/123"):
...
class TestWorkflowRunnerRun:
    def test_mark_workflow_failed_records_terminal_timestamp(self):
        runner = _make_runner()
        runner.workflow = _make_mock_workflow(is_complete=False)

        runner._mark_workflow_failed()

        assert runner.workflow.status == "failed"
        assert isinstance(runner.workflow.completion_time, float)

    def test_can_reflect_after_execute_failure(self):
        runner = _make_runner()
        execute_node = MagicMock()
        execute_node.type = "execute_sql"
        reflect_node = MagicMock()
        reflect_node.type = "reflect"
        runner.workflow = _make_mock_workflow(is_complete=False)
        runner.workflow.get_next_node.return_value = reflect_node

        assert runner._can_reflect_after_failure(execute_node) is True
    ...
    def test_run_with_completed_workflow(self, tmp_path):
        ...
        with patch("dataengineer.agent.workflow_runner.generate_workflow", return_value=mock_wf):
            with patch("dataengineer.agent.workflow_runner.get_trace_url", return_value=None):
                with patch("dataengineer.agent.workflow_runner.setup_node_input"):
    ...
    def test_run_max_steps_stops_loop(self, tmp_path):
        ...
        with patch("dataengineer.agent.workflow_runner.generate_workflow", return_value=mock_wf):
            with patch("dataengineer.agent.workflow_runner.evaluate_result", return_value={"success": True}):
                with patch("dataengineer.agent.workflow_runner.get_trace_url", return_value=None):
                    with patch("dataengineer.agent.workflow_runner.setup_node_input"):
