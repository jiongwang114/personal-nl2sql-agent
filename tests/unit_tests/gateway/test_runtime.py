...
"""Unit tests for dataengineer.gateway.runtime.DataEngineerGateway."""
...
from dataengineer.gateway.channel.base import ChannelAdapter
from dataengineer.gateway.models import OutboundMessage
...
def _make_gateway(channels_config: dict | None = None) -> DataEngineerGateway:
    agent_config = MagicMock()
    agent_config.channels_config = channels_config or {}
    gw = DataEngineerGateway(
...
class TestGatewayInit:
    ...
    def test_task_manager_created_with_non_interactive_mode(self):
        """DataEngineerGateway should create ChatTaskManager with default_interactive=False."""
        agent_config = MagicMock()
        with patch("dataengineer.gateway.runtime.ChatTaskManager") as mock_ctm:
            DataEngineerGateway(agent_config=agent_config, channels_config={})
...
class TestGatewayStart:
    ...
    async def test_no_enabled_channels_returns_early(self):
        gw = _make_gateway({})
        with patch("dataengineer.gateway.runtime.register_builtins"):
            await gw.start()
    ...
    async def test_enabled_channel_instantiated_and_started(self):
        ...
        with (
            patch("dataengineer.gateway.runtime.register_builtins"),
            patch("dataengineer.gateway.runtime.get_adapter_class", return_value=_FakeAdapter),
        ):
            ...
            async def _start_with_immediate_shutdown():
                from dataengineer.gateway.models import ChannelConfig
...
class TestGatewaySignals:
    ...
    async def test_windows_platform_check_in_code(self):
        ...
        import dataengineer.gateway.runtime as gw_mod

        source = inspect.getsource(gw_mod.DataEngineerGateway.start)
