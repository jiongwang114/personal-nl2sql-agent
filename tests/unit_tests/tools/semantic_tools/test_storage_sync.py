...
"""Unit tests for dataengineer/tools/semantic_tools/storage_sync.py"""
...
def _make_metric(name="revenue", path=None, description="Total revenue"):
    from dataengineer.tools.semantic_tools.models import MetricDefinition
...
class TestEnsureSemanticModelStore:
    def test_lazy_init_creates_store(self):
        ...
        with patch("dataengineer.storage.semantic_model.store.SemanticModelRAG", mock_rag_class):
...
class TestEnsureMetricStore:
    def test_lazy_init_creates_store(self):
        ...
        with patch("dataengineer.storage.metric.store.MetricRAG", mock_rag_class):
...
class TestEnsureSubjectTreeStore:
    def test_lazy_init_creates_store(self):
        ...
        with patch("dataengineer.storage.registry.get_subject_tree_store", return_value=mock_store):
...
class TestStoreSemanticModel:
    ...
    def test_stores_semantic_model_info_with_extra_metadata(self):
        from dataengineer.tools.semantic_tools.models import DimensionInfo, SemanticModelInfo
    ...
    def test_rejects_semantic_model_info_without_physical_table_name(self):
        from dataengineer.tools.semantic_tools.models import SemanticModelInfo
        from dataengineer.utils.exceptions import DataEngineerException

        manager = _make_manager()
        with pytest.raises(DataEngineerException, match="missing physical table_name"):
