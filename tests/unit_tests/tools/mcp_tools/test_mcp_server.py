...
"""
Unit tests for DataEngineer MCP Server.
...
from dataengineer.mcp_server import (
...
class TestMCPServerCreation:
    ...
    @pytest.fixture
    def server(self):
        with patch.object(DataEngineerMCPServer, "_init_tools"), patch.object(DataEngineerMCPServer, "_register_tools"):
...
class TestDynamicModeHTTPEndpoints:
    ...
    async def test_root_endpoint(self, http_app):
        ...
        async with httpx.AsyncClient(
            ...
            data = response.json()
            assert data["service"] == "DataEngineer MCP Server"
