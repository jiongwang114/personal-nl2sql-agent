...
"""Extended unit tests for dataengineer/tools/mcp_tools/mcp_tool.py"""
...
from dataengineer.tools.mcp_tools.mcp_config import ToolFilterConfig
from dataengineer.tools.mcp_tools.mcp_tool import MCPTool, _parse_header_from_parts, parse_command_string
...
def _make_mcp_tool(tmp_path: Path) -> MCPTool:
    ...
    with patch("dataengineer.tools.mcp_tools.mcp_tool.MCPManager", return_value=mock_manager):
        tool = MCPTool()
...
class TestParseCommandString:
    ...
    def test_unsupported_transport_raises(self):
        with pytest.raises(DataEngineerException, match="Unsupported transport"):
    ...
    def test_args_between_command_and_env(self):
        cmd = "--transport stdio myserver python -m dataengineer.main arg1 arg2 --env KEY=VAL"
        t, name, params = parse_command_string(cmd)
...
class TestMCPToolCheckConnectivity:
    def test_check_connectivity_success(self, tmp_path):
        tool = _make_mcp_tool(tmp_path)
        with patch("dataengineer.tools.mcp_tools.mcp_tool.run_async", return_value=(True, "OK", {"connected": True})):
    ...
    def test_check_connectivity_failure(self, tmp_path):
        tool = _make_mcp_tool(tmp_path)
        with patch("dataengineer.tools.mcp_tools.mcp_tool.run_async", return_value=(False, "Connection refused", {})):
            result = tool.check_connectivity("srv")
        assert result.success is False

    def test_check_connectivity_exception(self, tmp_path):
        tool = _make_mcp_tool(tmp_path)
        with patch("dataengineer.tools.mcp_tools.mcp_tool.run_async", side_effect=Exception("timeout")):
            result = tool.check_connectivity("srv")
        assert result.success is False
...
class TestMCPToolListTools:
    def test_list_tools_success(self, tmp_path):
        ...
        with patch(
            "dataengineer.tools.mcp_tools.mcp_tool.run_async",
    ...
    def test_list_tools_failure(self, tmp_path):
        tool = _make_mcp_tool(tmp_path)
        with patch("dataengineer.tools.mcp_tools.mcp_tool.run_async", return_value=(False, "Error", [])):
            result = tool.list_tools("srv")
        assert result.success is False
    ...
    def test_list_tools_exception(self, tmp_path):
        tool = _make_mcp_tool(tmp_path)
        with patch("dataengineer.tools.mcp_tools.mcp_tool.run_async", side_effect=Exception("boom")):
            result = tool.list_tools("srv")
        assert result.success is False
...
class TestMCPToolCallTool:
    def test_call_tool_success(self, tmp_path):
        ...
        with patch("dataengineer.tools.mcp_tools.mcp_tool.run_async", return_value=(True, "OK", result_data)):
    ...
    def test_call_tool_none_arguments(self, tmp_path):
        tool = _make_mcp_tool(tmp_path)
        with patch("dataengineer.tools.mcp_tools.mcp_tool.run_async", return_value=(True, "OK", {})):
            result = tool.call_tool("srv", "my_tool", None)
        assert result.success is True

    def test_call_tool_exception(self, tmp_path):
        tool = _make_mcp_tool(tmp_path)
        with patch("dataengineer.tools.mcp_tools.mcp_tool.run_async", side_effect=Exception("crash")):
            result = tool.call_tool("srv", "my_tool", {})
        assert result.success is False
