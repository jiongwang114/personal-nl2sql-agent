...
def date_parsing_tools(mock_agent_config, mock_model):
    with patch("dataengineer.tools.func_tool.date_parsing_tools.DateParserTool") as mock_parser_cls:
...
class TestGetLanguageSetting:
    def test_returns_en_when_no_nodes(self, mock_agent_config, mock_model):
        with patch("dataengineer.tools.func_tool.date_parsing_tools.DateParserTool"):
            from dataengineer.tools.func_tool.date_parsing_tools import DateParsingTools
...
class TestAvailableTools:
    def test_available_tools_returns_one_tool(self, date_parsing_tools):
        with patch("dataengineer.tools.func_tool.date_parsing_tools.trans_to_function_tool") as mock_trans:
...
class TestParseTemporalExpressions:
    def test_success(self, date_parsing_tools):
        ...
        with patch("dataengineer.utils.time_utils.get_default_current_date", return_value="2024-02-01"):
    ...
    def test_success_no_current_date(self, date_parsing_tools):
        ...
        with patch("dataengineer.utils.time_utils.get_default_current_date", return_value="2024-02-15"):
    ...
    def test_uses_reference_date_as_fallback(self, date_parsing_tools):
        ...
        with patch("dataengineer.utils.time_utils.get_default_current_date", return_value="2023-06-15") as mock_get_date:
...
class TestGetCurrentDate:
    def test_returns_current_date_no_reference(self, date_parsing_tools):
        with patch("dataengineer.utils.time_utils.get_default_current_date", return_value="2024-03-15"):
    ...
    def test_returns_reference_date_when_set(self, date_parsing_tools):
        ...
        with patch("dataengineer.utils.time_utils.get_default_current_date", return_value="2023-12-31"):
            result = date_parsing_tools.get_current_date()
