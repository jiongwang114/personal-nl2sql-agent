...
"""Unit tests for dataengineer/tools/func_tool/gen_semantic_model_tools.py"""
...
class TestAnalyzeJoinPatterns:
    ...
    def test_finds_join_pattern(self):
        ...
        with patch("dataengineer.storage.reference_sql.store.ReferenceSqlRAG", return_value=mock_rag):
            tools = _make_tools(db_tool)
            result = tools._analyze_join_patterns_from_history(["orders", "customers"], 10)
        assert len(result) >= 1
