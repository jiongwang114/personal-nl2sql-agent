...
# the `from dataengineer.tools.func_tool.scheduler_tools import ...` below transitively
...
class TestGetAdapter:
    ...
    def test_success_with_mocked_registry(self):
        ...
        with patch("datus_scheduler_core.registry.SchedulerAdapterRegistry", mock_registry):
...
class TestSchedulerToolsAcceptance:
    ...
    def test_submit_trigger_poll_and_read_back_sql_job(self, tmp_path):
        ...
        with patch.object(tools, "_get_adapter", return_value=fake_adapter):
            submit = tools.submit_sql_job(
                "Daily Sales", str(sql_file), conn_id="warehouse", schedule="0 8 * * *"
            )
...
class TestStrictSqlPathResolution:
    ...
    def test_allowlisted_sql_accepted_in_strict_mode(self, tmp_path):
        from dataengineer.tools.func_tool.fs_path_policy import PathAllowlist
