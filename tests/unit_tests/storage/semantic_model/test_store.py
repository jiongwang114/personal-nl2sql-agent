...
"""Tests for dataengineer/storage/semantic_model/store.py -- SemanticModelStorage and SemanticModelRAG."""
...
from dataengineer.storage.semantic_model.store import SemanticModelRAG, SemanticModelStorage
...
class TestUpdateEntryYamlSync:
    ...
    def test_update_entry_nonexistent_raises(self, sem_storage):
        """update_entry raises DataEngineerException when the entry_id does not exist."""
        with pytest.raises(DataEngineerException, match="entry not found"):
            sem_storage.update_entry("table:nonexistent", {"description": "x"})

    def test_update_entry_empty_id_raises(self, sem_storage):
        """update_entry raises DataEngineerException when entry_id is empty."""
        with pytest.raises(DataEngineerException, match="entry_id must not be empty"):
            sem_storage.update_entry("", {"description": "x"})

    def test_update_entry_empty_values_raises(self, sem_storage):
        """update_entry raises DataEngineerException when update_values is empty."""
        table_obj = _make_table_object("orders")
        sem_storage.store_batch([table_obj])

        with pytest.raises(DataEngineerException, match="update_values must not be empty"):
