...
"""Unit tests for dataengineer.storage.storage_cfg."""

import os

import pytest

from dataengineer.storage.embedding_models import DEFAULT_MODEL_CONFIG, EmbeddingModel
from dataengineer.storage.storage_cfg import (
...
class TestSaveStorageConfigs:
    ...
    def test_save_configs_creates_directory(self, tmp_path):
        ...
        assert os.path.isfile(os.path.join(rag_path, "dataengineer_db.cfg"))
...
class TestCheckStorageConfig:
    ...
    def test_check_raises_on_mismatch(self, tmp_path):
        """Mismatching config should raise DataEngineerException."""
        rag_path = str(tmp_path / "rag")
