...
"""Unit tests for dataengineer.storage.document.doc_init."""
...
class TestInitPlatformDocsDbPathDeprecation:
    ...
    def test_db_path_emits_deprecation_warning(self):
        ...
        with (
            ...
            patch("dataengineer.storage.document.doc_init.document_store") as mock_store_fn,
        ):
...
class TestInitPlatformDocsEmit:
    ...
    @patch("dataengineer.storage.document.doc_init.document_store")
    ...
    def test_emit_task_failed_on_store_error(self, mock_store_fn):
        ...
        mock_store_fn.side_effect = DataEngineerException(ErrorCode.COMMON_VALIDATION_FAILED)
    ...
    def test_emit_task_failed_on_processing_exception(self, mock_store_fn):
        ...
        with inner_patch("dataengineer.storage.document.doc_init.LocalFetcher") as mock_fetcher_cls:
...
class TestInitPlatformDocsOverwrite:
    ...
    @patch("dataengineer.storage.document.doc_init.StreamingDocProcessor")
    @patch("dataengineer.storage.document.doc_init.LocalFetcher")
    @patch("dataengineer.storage.document.doc_init.document_store")
    def test_overwrite_local_source_processes_documents(self, mock_store_fn, mock_fetcher_cls, mock_processor_cls):
        ...
        from dataengineer.storage.document.streaming_processor import ProcessingStats
    ...
    @patch("dataengineer.storage.document.doc_init.StreamingDocProcessor")
    @patch("dataengineer.storage.document.doc_init.GitHubFetcher")
    @patch("dataengineer.storage.document.doc_init.document_store")
    ...
    @patch("dataengineer.storage.document.doc_init.StreamingDocProcessor")
    @patch("dataengineer.storage.document.doc_init.WebFetcher")
    @patch("dataengineer.storage.document.doc_init.document_store")
