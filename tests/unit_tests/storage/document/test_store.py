...
"""Tests for dataengineer/storage/document/store.py -- DocumentStore."""

from unittest.mock import patch

import pytest

from dataengineer.storage.document.schemas import PlatformDocChunk
from dataengineer.storage.document.store import DocumentStore, document_store, get_platform_doc_schema
from dataengineer.storage.embedding_models import get_document_embedding_model
...
class TestValidateIdentifier:
    ...
    def test_invalid_with_semicolon(self):
        """String with semicolon should raise DataEngineerException."""
        with pytest.raises(DataEngineerException, match="Invalid version"):
            DocumentStore._validate_identifier("v1; DROP TABLE", "version")

    def test_invalid_with_quotes(self):
        """String with quotes should raise DataEngineerException."""
        with pytest.raises(DataEngineerException, match="Invalid version"):
            DocumentStore._validate_identifier("v1' OR '1'='1", "version")

    def test_invalid_with_parentheses(self):
        """String with parentheses should raise DataEngineerException."""
        with pytest.raises(DataEngineerException, match="Invalid version"):
...
class TestDocumentStoreDeleteDocs:
    ...
    def test_delete_docs_invalid_version_raises(self, doc_store):
        """Deleting with unsafe version string should raise DataEngineerException."""
...
class TestDocumentStoreFactory:
    ...
    def test_invalid_platform_empty(self):
        """Empty platform should raise DataEngineerException."""
        with pytest.raises(DataEngineerException, match="Invalid platform name"):
            document_store("")

    def test_invalid_platform_special_chars(self):
        """Platform with spaces/dots should raise DataEngineerException."""
        with pytest.raises(DataEngineerException, match="Invalid platform name"):
            document_store("my platform.v1")

    def test_invalid_platform_sql_injection(self):
        """Platform with SQL injection attempt should raise DataEngineerException."""
        with pytest.raises(DataEngineerException, match="Invalid platform name"):
