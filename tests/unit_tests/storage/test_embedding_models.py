...
"""Tests for dataengineer/storage/embedding_models.py — EmbeddingModel and get_embedding_model."""
...
class TestErrorMessagePreservation:
    ...
    def test_model_property_raises_with_error_message(self):
        """Accessing .model on a failed model should raise DataEngineerException containing the error message."""
        model = EmbeddingModel(model_name="bad-model", dim_size=384)
        model.is_model_failed = True
...
class TestDeviceSetting:
    ...
    def test_default_device_is_global(self):
        ...
        import dataengineer.storage.embedding_models as em_mod
