...
from dataengineer.storage.base import BaseEmbeddingStore
...
class TestGetStorageLRUCache:
    ...
    def test_same_project_returns_cached(self, reset_global_singletons):
        ...
        from dataengineer.storage.registry import get_storage

        def _factory(embedding_model, **kwargs):
            return BaseEmbeddingStore(table_name="test", embedding_model=embedding_model, **kwargs)

        with patch("dataengineer.storage.registry.get_embedding_model", return_value=_FakeEmbeddingModel()):
    ...
    def test_different_project_returns_different(self, reset_global_singletons):
        ...
        with (
            patch("dataengineer.storage.registry.get_embedding_model", return_value=_FakeEmbeddingModel()),
            patch("dataengineer.storage.backend_holder.get_vector_backend") as mock_backend,
        ):
    ...
    def test_empty_project_raises(self, reset_global_singletons):
        """get_storage with empty project propagates the backend DataEngineerException."""
        import pytest
    ...
    def test_clear_registry_clears_cache(self, reset_global_singletons):
        ...
        from dataengineer.storage.registry import _get_storage_cached, clear_storage_registry, get_storage
    ...
    def test_subject_store_binds_subject_tree_by_requested_project(self, reset_global_singletons):
        ...
        from dataengineer.storage.metric.store import MetricStorage
        from dataengineer.storage.registry import get_storage
        ...
        with (
            patch("dataengineer.storage.registry.get_embedding_model", return_value=_FakeEmbeddingModel()),
            patch("dataengineer.storage.registry._get_subject_tree_cached", return_value=project_tree) as mock_tree,
            patch("dataengineer.storage.backend_holder.get_vector_backend") as mock_backend,
        ):
            mock_backend.return_value = MagicMock()
...
class TestPreloadAllStorages:
    ...
    def test_preload_forwards_project(self, reset_global_singletons):
        ...
        from dataengineer.storage.registry import preload_all_storages

        with (
            patch("dataengineer.storage.registry.get_storage") as mock_get_storage,
            patch("dataengineer.storage.backend_holder.init_backends") as mock_init,
            patch("dataengineer.storage.registry.get_subject_tree_store"),
        ):
    ...
    def test_preload_applies_defaults(self, reset_global_singletons):
        ...
        from dataengineer.storage.registry import get_storage_defaults, preload_all_storages

        with (
            patch("dataengineer.storage.registry.get_storage"),
            patch("dataengineer.storage.backend_holder.init_backends"),
            patch("dataengineer.storage.registry.get_subject_tree_store"),
        ):
