...
from dataengineer.storage.rdb.sqlite_backend import SqliteRdbDatabase
...
class TestSqliteRdbDatabaseEdgeCases:
    ...
    def test_connection_error_raises_datus_exception(self, tmp_path):
        """Connecting to an invalid db file path raises DataEngineerException."""
...
class TestSqliteRdbBackendConnect:
    ...
    def test_connect_builds_project_scoped_path(self, tmp_path):
        """connect(project, store) places the store under ``{data_dir}/{project}/dataengineer_db/``."""
        ...
        assert db.db_file == os.path.join(str(tmp_path), "proj_a", "dataengineer_db", "test.db")
    ...
    def test_single_instance_reused_across_projects(self, tmp_path):
        ...
        assert db_a.db_file == os.path.join(str(tmp_path), "proj_a", "dataengineer_db", "test.db")
        assert db_b.db_file == os.path.join(str(tmp_path), "proj_b", "dataengineer_db", "test.db")
