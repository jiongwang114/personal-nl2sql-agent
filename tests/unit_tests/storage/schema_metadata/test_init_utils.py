...
"""Unit tests for dataengineer.storage.schema_metadata.init_utils."""
...
from dataengineer.storage.schema_metadata.init_utils import exists_table_value
...
class TestExistsTableValueExceptionHandling:
    """Exception from storage should be wrapped in DataEngineerException."""

    @pytest.mark.ci
    def test_search_all_schemas_error_raises_datus_exception(self):
        """When search_all_schemas raises, should wrap in DataEngineerException."""
        storage = MagicMock()
    ...
    def test_search_all_value_error_raises_datus_exception(self):
        """When search_all_value raises, should wrap in DataEngineerException."""
        storage = MagicMock()
        storage.search_all_schemas.return_value = _make_schema_table([], [])
