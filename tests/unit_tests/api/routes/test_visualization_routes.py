...
"""Unit tests for dataengineer/api/routes/visualization_routes.py — CI level, zero external deps."""
...
from dataengineer.api.routes.visualization_routes import router
...
def _client_with(generate_return):
    ...
    from dataengineer.api.deps import get_dataengineer_service

    app = _make_app()
    app.dependency_overrides[get_dataengineer_service] = lambda: _mock_svc(generate_return)
