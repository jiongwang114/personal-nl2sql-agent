...
from dataengineer.api.legacy_models import (
    FeedbackRequest,
    FeedbackResponse,
    FeedbackStatus,
    HealthResponse,
...
def app(mock_agent_config):
    ...
    with (
        patch("dataengineer.api.service.load_agent_config", return_value=mock_agent_config),
        patch("dataengineer.api.service.TaskStore", mock_task_store_cls),
        patch("dataengineer.api.service.Agent", mock_agent_cls),
    ):
...
def auth_token():
    ...
    from dataengineer.api.legacy_auth import auth_service

    token_data = auth_service.generate_access_token("dataengineer_client")
