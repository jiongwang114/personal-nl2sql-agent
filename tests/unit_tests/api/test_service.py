...
"""
Unit tests for dataengineer/api/service.py — DataEngineerAPIService and helper functions.
...
from dataengineer.api.legacy_models import FeedbackRequest, FeedbackStatus, RunWorkflowRequest
from dataengineer.api.service import DataEngineerAPIService
...
def _make_service(args=None):
    return DataEngineerAPIService(args=args or _make_args())
...
# ---------------------------------------------------------------------------
# DataEngineerAPIService.__init__
# ---------------------------------------------------------------------------


class TestDataEngineerAPIServiceInit:
...
class TestDataEngineerAPIServiceInitialize:
    @pytest.mark.asyncio
    async def test_initialize_sets_config_and_task_store(self):
        ...
        with patch("dataengineer.api.service.load_agent_config", return_value=mock_cfg):
            with patch("dataengineer.api.service.TaskStore", return_value=mock_task_store):
...
class TestHealthCheck:
    ...
    async def test_unhealthy_on_exception(self):
        ...
        with patch("dataengineer.api.service.Agent", side_effect=RuntimeError("config error")):
            response = await service.health_check()
