...
from dataengineer.api.deps import get_dataengineer_service, init_deps
...
class TestInitDeps:
    ...
    def test_sets_auth_provider(self):
        ...
        mock_cache = MagicMock(spec=DataEngineerServiceCache)
...
@pytest.mark.asyncio
class TestGetDataEngineerService:
    """Tests for get_dataengineer_service — FastAPI dependency."""
    ...
    async def test_authenticates_and_returns_service(self):
        ...
        request.state = MagicMock()
        with patch(
            "dataengineer.api.deps.DataEngineerService.compute_fingerprint",
            return_value="fp-xyz",
        ) as mock_fp:
            result = await get_dataengineer_service(request)
    ...
    async def test_factory_propagates_default_source_and_interactive(self):
        """Factory passes module-level defaults through to DataEngineerService constructor."""
        from unittest.mock import patch
        ...
        with (
            ...
            patch("dataengineer.api.deps.DataEngineerService") as mock_svc_cls,
        ):
    ...
    async def test_factory_loads_config_when_none(self, real_agent_config):
        ...
        try:
            result = await get_dataengineer_service(request)
            # If it succeeds, result should be a DataEngineerService
            assert isinstance(result, DataEngineerService)
