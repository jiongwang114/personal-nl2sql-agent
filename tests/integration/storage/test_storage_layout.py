...
Validates the end-to-end wiring of ``AgentConfig`` + ``DataEngineerPathManager``:
...
* Live backends (``create_vector_connection`` / ``create_rdb_for_store``)
  actually write under ``{data_dir}/{project}/dataengineer_db`` — this catches
...
from dataengineer.configuration.agent_config import AgentConfig, NodeConfig, _normalize_project_name
...
class TestStorageLayoutIntegration:
    ...
    def test_kb_subtree_follows_project_root(self, tmp_path):
        ...
        # The project-level skills directory lives under project_root/.dataengineer/skills
...
class TestBackendLandingPath:
    ...
    ``DataEngineerPathManager.data_dir`` and the path the backends actually open.
    """
    ...
    def test_vector_connection_lands_under_project_shard(self, tmp_path, _backends):
        ...
        try:
            expected = data_dir / "proj_x" / "dataengineer_db"
    ...
    def test_rdb_store_file_lands_under_project_shard(self, tmp_path, _backends):
        ...
        expected = data_dir / "proj_x" / "dataengineer_db" / "subject_tree.db"
