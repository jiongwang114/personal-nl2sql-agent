...
from dataengineer.api.legacy_models import FeedbackResponse, RunWorkflowResponse
from dataengineer.api.service import DataEngineerAPIService, create_app


def _get_service_module():
    """Get the actual service module via sys.modules to avoid name shadowing."""
    return sys.modules["dataengineer.api.service"]
...
async def api_client(app, agent_args):
    ...
    mock_service = DataEngineerAPIService(agent_args)
...
async def auth_token(api_client):
    ...
    resp = await api_client.post(
        "/auth/token",
        data={
            "client_id": "dataengineer_client",
            "client_secret": "dataengineer_secret_key",
...
class TestAPI:
    ...
    async def test_sync_workflow(self, authenticated_client):
        ...
        with patch.object(DataEngineerAPIService, "run_workflow") as mock_run:
    ...
    async def test_stream_workflow(self, authenticated_client):
        ...
        with patch.object(DataEngineerAPIService, "run_workflow_stream") as mock_stream:
    ...
    async def test_feedback(self, authenticated_client):
        ...
        with patch.object(DataEngineerAPIService, "record_feedback") as mock_feedback:
