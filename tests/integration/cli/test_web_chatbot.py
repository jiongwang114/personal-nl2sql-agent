...
Tests ChatExecutor and ConfigManager with real DataEngineerCLI + real LLM,
...
from dataengineer.cli.web.chat_executor import ChatExecutor
from dataengineer.cli.web.config_manager import ConfigManager, create_cli_args, get_available_datasources
...
class TestConfigManagerIntegration:
    ...
    def test_setup_config_and_models(self):
        """W1-04: ConfigManager.setup_config returns DataEngineerCLI with models available."""
        cm = ConfigManager()
        cli = cm.setup_config(config_path=TESTS_CONF, datasource="ssb_sqlite")

        assert cli is not None, "setup_config should return a DataEngineerCLI instance"
...
class TestChatExecutorIntegration:
    """Integration tests for ChatExecutor with real DataEngineerCLI."""
    ...
    def test_execute_chat_stream_produces_actions(self, mock_args):
        """W2-02: ChatExecutor produces actions with real DataEngineerCLI + real LLM."""
        from dataengineer.cli.repl import DataEngineerCLI
        from tests.integration.conftest import wait_for_agent

        with (
            patch("dataengineer.cli.repl.PromptSession.prompt") as mock_prompt,
            patch("dataengineer.cli.repl.DataEngineerCLI.prompt_input") as mock_internal,
            patch("dataengineer.cli.repl.AtReferenceCompleter.parse_at_context") as at_data,
        ):
            mock_prompt.side_effect = [EOFError]
            mock_internal.side_effect = ["n"]
            at_data.return_value = [], [], []
            cli = DataEngineerCLI(args=mock_args)
            wait_for_agent(cli)
