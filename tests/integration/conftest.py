...
from dataengineer.tools.skill_tools import SkillConfig, SkillFuncTool, SkillManager
...
def llm_agent_config(tmp_path_factory) -> AgentConfig:
    ...
    and real skills from ~/.dataengineer/skills/.

    Skips if prerequisites missing:
    - DEEPSEEK_API_KEY not set
    - california_schools.sqlite not found
    - ~/.dataengineer/skills/report-generator/ not found
    """
...
@pytest.fixture
def mock_args():
    """Provides default mock arguments for initializing DataEngineerCLI."""
    return Namespace(
        history_file="~/.dataengineer/reference_sql",
        debug=False,
        datasource="bird_school",
