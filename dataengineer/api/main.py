...
"""
DataEngineer Agent FastAPI server startup script.

Single entry point for both the ``dataengineer-api`` console script and
``python -m dataengineer.api.main``. Supports foreground and daemon
...
from dataengineer.configuration.agent_config_loader import parse_config_path
...
def _default_paths(config_path: str = "") -> Tuple[Path, Path]:
    ...
    from dataengineer.configuration.agent_config_loader import get_agent_home
    from dataengineer.utils.path_manager import DataEngineerPathManager

    path_manager = DataEngineerPathManager(get_agent_home(config_path))
    pid_file = path_manager.pid_file_path("dataengineer-agent-api")
    log_file = path_manager.logs_dir / "dataengineer-agent-api.log"
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="DataEngineer Agent FastAPI Server")
    parser.add_argument("-v", "--version", action="version", version=f"DataEngineer API {__version__}")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Agent configuration file (default: ./conf/agent.yml > ~/.dataengineer/conf/agent.yml)",
    )
    parser.add_argument(
        "--datasource",
        type=str,
        default=os.getenv("DATAENGINEER_DATASOURCE", "default"),
        help="Datasource identifier (default: DATAENGINEER_DATASOURCE env or 'default')",
    )
    parser.add_argument(
        "--output-dir",
        dest="output_dir",
        type=str,
        default=os.getenv("DATAENGINEER_OUTPUT_DIR", "./output"),
    ...
    parser.add_argument(
        ...
        dest="log_level",
        type=str,
        default=os.getenv("DATAENGINEER_LOG_LEVEL", "INFO"),
        choices=["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"],
        help="Log level (default: INFO or DATAENGINEER_LOG_LEVEL env var)",
    )
    ...
    parser.add_argument(
        "--pid-file",
        type=str,
        help="PID file path (default: ~/.dataengineer/run/dataengineer-agent-api.pid)",
    )
    parser.add_argument(
        "--daemon-log-file",
        type=str,
        help="Daemon log file path (default: ~/.dataengineer/logs/dataengineer-agent-api.log)",
    )
    return parser


def main():
    """Main entry point for starting the DataEngineer Agent API server."""
    ...
    os.environ["DATAENGINEER_CONFIG"] = config_path
    os.environ["DATAENGINEER_DATASOURCE"] = args.datasource
    os.environ["DATAENGINEER_OUTPUT_DIR"] = args.output_dir
    os.environ["DATAENGINEER_LOG_LEVEL"] = args.log_level
    ...
    if args.daemon:
        ...
        configure_logging(args.debug, log_dir=str(log_file.parent), console_output=False)
        logger.info(
            f"Starting DataEngineer Agent API server (daemon) on {args.host}:{args.port} | "
    ...
    logger.info(f"Starting DataEngineer Agent API server on {args.host}:{args.port}")
