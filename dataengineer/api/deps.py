"""FastAPI dependency injection — plugin-based auth + DataEngineerService cache."""
...
from dataengineer.api.auth.provider import AuthProvider
...
_service_cache: Optional[DataEngineerServiceCache] = None
...
async def get_dataengineer_service(request: Request) -> DataEngineerService:
    ...
    then returns a cached-per-project DataEngineerService. If AppContext has no
    ...
    expected_fp = DataEngineerService.compute_fingerprint(ctx.config) if ctx.config is not None else None
    cache_key = ctx.project_id or _DEFAULT_PROJECT_KEY

    async def _factory() -> DataEngineerService:
...
def get_app_context(request: Request) -> AppContext:
    """Return the ``AppContext`` cached on the request by ``get_dataengineer_service``.
...
ServiceDep = Annotated[DataEngineerService, Depends(get_dataengineer_service)]
