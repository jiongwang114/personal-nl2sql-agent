...
from dataengineer.api.deps import init_deps
from dataengineer.api.services.dataengineer_service_cache import DataEngineerServiceCache
from dataengineer.configuration.agent_config_loader import load_agent_config, parse_config_path
...
from dataengineer.storage.task import TaskStore
...
class DataEngineerAPIService:
    """Main service class for DataEngineer Agent API."""
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.warning(
                ...
                "until a model is set up via 'dataengineer' CLI + /model command."
            )
...
async def lifespan(app: FastAPI):
    ...
    service = DataEngineerAPIService(args)

    # Startup
    await service.initialize()

    # Initialize plugin-based auth and service cache for new API routes
    datasource = getattr(args, "datasource", None) or os.getenv("DATAENGINEER_DATASOURCE", "default")
    ...
    service_cache = DataEngineerServiceCache(max_size=128)
    init_deps(
        auth_provider,
        service_cache,
    ...
    logger.info("DataEngineer API Service started")
    yield
    # Shutdown
    logger.info("DataEngineer API Service shutting down")


def create_app(agent_args: argparse.Namespace) -> FastAPI:
    """Create FastAPI app with agent args."""
    app = FastAPI(
        title="DataEngineer Agent API",
        description="FastAPI service for DataEngineer Agent workflow execution",
        version="1.0.0",
        lifespan=lifespan,
    )
    ...
    # Add CORS middleware
    cors_origins_env = os.getenv("DATAENGINEER_CORS_ORIGINS", "*")
    ...
    _route_modules = [
        ("dataengineer.api.routes.chat_routes", "chat"),
        ("dataengineer.api.routes.cli_routes", "cli"),
        ("dataengineer.api.routes.database_routes", "database"),
        ...
        ("dataengineer.api.routes.agent_routes", "agent"),
        ("dataengineer.api.routes.visualization_routes", "visualization"),
        ("dataengineer.api.routes.tool_routes", "tool"),
        ("dataengineer.api.routes.success_story_routes", "success_story"),
    ]
    import importlib
...
# Configuration via environment variables: DATAENGINEER_CONFIG, DATAENGINEER_DATASOURCE, DATAENGINEER_OUTPUT_DIR, DATAENGINEER_LOG_LEVEL
_config_file = os.getenv("DATAENGINEER_CONFIG", "")  # Empty string uses default resolution
try:
...
_datasource = os.getenv("DATAENGINEER_DATASOURCE", "default")
_output_dir = os.getenv("DATAENGINEER_OUTPUT_DIR", "./output")
_log_level = os.getenv("DATAENGINEER_LOG_LEVEL", "INFO")
