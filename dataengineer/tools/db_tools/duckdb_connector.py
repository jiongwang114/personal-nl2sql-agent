...
from dataengineer.schemas.base import TABLE_TYPE
from dataengineer.schemas.node_models import ExecuteSQLResult
from dataengineer.tools.db_tools._migration_compat import MigrationTargetMixin
...
def _metadata_names(_type: str) -> _DBMetadataNames:
    if _type not in METADATA_DICT:
        raise DataEngineerException(ErrorCode.COMMON_FIELD_INVALID, f"Invalid type `{_type}` for Database table type")
...
class DuckdbConnector(BaseSqlConnector, SchemaNamespaceMixin, MigrationTargetMixin):
    ...
    def __init__(self, config: DuckDBConfig):
        ...
        if config.database_name:
            ...
            from dataengineer.configuration.agent_config import file_stem_from_uri
    ...
    def _handle_exception(self, e: Exception, sql: str = "") -> DataEngineerException:
    ...
    def execute_content_set(self, sql_query: str) -> ExecuteSQLResult:
        ...
        try:
            ...
            from dataengineer.utils.sql_utils import parse_context_switch
    ...
    def get_schema(
        ...
        try:
            ...
            if "error_message" in e.message_args:
                ...
                message = e.message
            raise DataEngineerException(ErrorCode.DB_QUERY_METADATA_FAILED, message=message)
        except Exception as e:
            raise DataEngineerException(
