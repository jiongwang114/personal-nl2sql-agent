...
def _resolve_home(datus_home: Optional[Path]) -> Path:
    """Resolve the effective ``~/.dataengineer`` root for whitelist anchors."""
    ...
    *,
    ...
    Args:
        ...
        current_node: Name of the currently executing node. Only then does
            ``{root_path}/.dataengineer/memory/{current_node}/**`` qualify as
            ``WHITELIST``. ``None`` causes the memory anchor to be dropped,
            which demotes every ``.dataengineer/memory/**`` path to ``HIDDEN``.
        datus_home: Override for ``~/.dataengineer``. Primarily a test hook; when
    ...
    if any(_is_relative_to(resolved, anchor) for anchor in whitelist_anchors):
        ...
        if _is_relative_to(resolved, root_resolved):
            ...
            # rebuild the display with the canonical ``~/.dataengineer/`` prefix the
...
def build_walk_patterns(
    *,
    ...
    subtrees under ``.dataengineer/`` stay visible.
