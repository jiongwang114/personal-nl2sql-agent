...
"""Project-level ``./.dataengineer/config.yml`` override.
...
@dataclass
class ProjectTarget:
    """Structured ``target:`` value from ``./.dataengineer/config.yml``.
...
class ProjectOverride:
    """In-memory representation of ``./.dataengineer/config.yml``.
...
def load_project_override(cwd: Optional[str] = None) -> Optional[ProjectOverride]:
    """Read ``./.dataengineer/config.yml`` relative to ``cwd``.
...
def save_project_override(override: ProjectOverride, cwd: Optional[str] = None) -> Path:
    """Write ``override`` to ``./.dataengineer/config.yml``.

    Creates the ``.dataengineer/`` parent directory if missing.  ``None`` fields
