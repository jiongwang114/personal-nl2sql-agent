...
from dataengineer.configuration.project_config import ProjectTarget, load_project_override
...
def parse_config_path(config_file: str = "", create_if_missing: bool = False) -> Path:
    ...
    3. ~/.dataengineer/conf/agent.yml (fixed path, not from agent.home config)

    When ``create_if_missing`` is True and no config file is found, a minimal
    agent.yml is bootstrapped at ``~/.dataengineer/conf/agent.yml`` instead of raising.
    ...
    Raises:
        DataEngineerException: If configuration file not found (and create_if_missing is False)
    """
    ...
    # 3. Check default home directory (~/.dataengineer/conf/agent.yml)
    ...
    raise DataEngineerException(
        ...
        message=(
            "Agent configuration file not found. Please configure your `conf/agent.yaml` or `.dataengineer/conf/agent.yml`"
...
def _apply_project_override(agent_raw: Dict[str, Any]) -> None:
    """Merge ``./.dataengineer/config.yml`` overlay into the raw agent config dict.
    ...
    web, ``dataengineer-api``, SDK), not just the CLI layer.
        ...
        if isinstance(override.target, ProjectTarget):
            if override.target.custom:
                ...
                if override.target.custom not in models:
                    logger.warning(
                        "target.custom '%s' from .dataengineer/config.yml not found in agent.models; "
            ...
            if override.target not in models:
                logger.warning(
                    "target '%s' from .dataengineer/config.yml not found in agent.models; will be resolved lazily at runtime.",
                    override.target,
                )
    ...
    if override.default_datasource is not None:
        ...
        if override.default_datasource not in datasources:
            raise DataEngineerException(
                ...
                message_args={
                    "field_name": "default_datasource (from .dataengineer/config.yml)",
...
def load_agent_config(reload: bool = False, create_if_missing: bool = False, **kwargs) -> AgentConfig:
    # Check config file in order: kwargs["config"] > conf/agent.yml > ~/.dataengineer/conf/agent.yml
    ...
    #   1. ``./.dataengineer/config.yml::default_datasource`` (via _apply_project_override)
            ...
            raise DataEngineerException(
                ...
                message_args={
                    "config_error": (
                        ...
                        "./.dataengineer/config.yml is missing and agent.yml has multiple "
                        "datasources without any marked as `default: true`. Run "
                        "`datus` in this project directory first to launch the "
                        "init wizard (which writes ./.dataengineer/config.yml with your "
...
def get_agent_home(config_file: str = "") -> str:
    ...
    return raw.get("agent", {}).get("home", "~/.dataengineer")
