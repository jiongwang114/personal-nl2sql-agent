...
from dataengineer.schemas.base import BaseInput
from dataengineer.schemas.node_models import StrategyType
from dataengineer.storage.embedding_models import init_embedding_models
from dataengineer.storage.storage_cfg import check_storage_config, save_storage_config
from dataengineer.utils.constants import DBType
...
from dataengineer.utils.path_utils import get_files_from_glob_pattern
...
# backend init to raise ``DataEngineerException``, so the CWD normalizer sanitizes
...
def _validate_project_name(value: str) -> str:
    ...
    Raises:
        DataEngineerException: when ``value`` contains forbidden characters or is
...
class AgentConfig:
    ...
    def __init__(self, nodes: Dict[str, NodeConfig], **kwargs):
        ...
        self.home = kwargs.get("home", "~/.dataengineer")
        ...
        if storage_config:
            if self._skip_init_dirs:
                ...
                except (DataEngineerException, KeyError, ValueError) as e:
        ...
        from dataengineer.storage.backend_holder import init_backends
        ...
        # Initialize channels configuration for DataEngineer Gateway IM gateway
    ...
    def _init_permissions_config(self, permissions_raw: Dict[str, Any]):
        ...
        try:
            from dataengineer.tools.permission.permission_config import PermissionConfig
    ...
    def _init_skills_config(self, skills_raw: Dict[str, Any]):
        ...
        try:
            from dataengineer.tools.skill_tools.skill_config import SkillConfig
    ...
    def save_run_dir(self, database: str, run_id: Optional[str] = None) -> Path:
        from dataengineer.utils.path_manager import DataEngineerPathManager

        return DataEngineerPathManager.resolve_run_dir(Path(self._save_dir), database, run_id)
    ...
    def trajectory_run_dir(self, database: str, run_id: Optional[str] = None) -> Path:
        ...
        return DataEngineerPathManager.resolve_run_dir(Path(self._trajectory_dir), database, run_id)
    ...
    def _init_dirs(
        ...
        logger.info(f"Using dataengineer home directory: {path_manager.datus_home}")
    ...
    def override_by_args(self, **kwargs):
        ...
        if kwargs.get("benchmark", ""):
            ...
            if benchmark_platform == "spider2" and self.db_type != "snowflake":
                raise DataEngineerException(code=ErrorCode.COMMON_UNSUPPORTED, message="spider2 only support snowflake")
            if benchmark_platform == "bird_dev" and self.db_type != DBType.SQLITE:
                raise DataEngineerException(code=ErrorCode.COMMON_UNSUPPORTED, message="bird_dev only support sqlite")
    ...
    def _set_path_manager(self, home: str) -> None:
        from dataengineer.utils.path_manager import DataEngineerPathManager, set_current_path_manager

        self.path_manager = DataEngineerPathManager(
            home,
    ...
    def active_model(self) -> ModelConfig:
        ...
        1. **Provider-level** (``self._target_provider`` + ``self._target_model``
           set via ``./.dataengineer/config.yml``) — synthesize a ``ModelConfig`` by
        ...
        3. Neither configured → raise :class:`DataEngineerException`.
        ...
        raise DataEngineerException(
            ...
            message=(
                ...
                "Run the DataEngineer CLI and use the /model command to set up a provider and model:\n"
                "  1. Start CLI: dataengineer\n"
                "  2. Use /model to pick a provider and model\n"
                "Or run 'dataengineer init' for first-time setup."
            ),
        )
    ...
    def set_active_provider_model(self, provider: str, model: str, persist: bool = True) -> None:
        ...
        ``True`` (default), writes the selection to ``./.dataengineer/config.yml``
    ...
    def set_provider_config(
        ...
        The disk write targets the global ``~/.dataengineer/conf/agent.yml`` under
    ...
    def provider_available(self, provider: str) -> bool:
        ...
        if auth_type == "subscription":
            try:
                from dataengineer.auth.claude_credential import get_claude_subscription_token
        ...
        if auth_type == "oauth":
            try:
                from dataengineer.auth.oauth_manager import OAuthManager
    ...
    def rag_storage_path(self) -> str:
        ...
        # so all isolation modes converge on the same ``dataengineer_db`` subdirectory.
        return os.path.join(self.rag_base_path, "dataengineer_db")
    ...
    def _is_file_based_vector_backend(self) -> bool:
        ...
        The ``dataengineer_db.cfg`` embedding-config check is only meaningful for
...
def _load_provider_catalog() -> Dict[str, Any]:
    ...
    try:
        ...
        from dataengineer.cli.provider_model_catalog import resolve_provider_models
        from dataengineer.utils.resource_utils import read_data_file_text
