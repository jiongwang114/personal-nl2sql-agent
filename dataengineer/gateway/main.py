...
"""
CLI entry point for the DataEngineer Gateway IM gateway.
...
_SERVICE_NAME = "dataengineer-gateway"
...
def _run_gateway(args: argparse.Namespace) -> None:
    """Load config and start the DataEngineer Gateway (blocking)."""
    ...
    from dataengineer.gateway.runtime import DataEngineerGateway

    gateway = DataEngineerGateway(
    ...
    logger.info("Starting DataEngineer Gateway...")
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="DataEngineer Gateway — IM Channel Gateway")
    parser.add_argument("-v", "--version", action="version", version=f"DataEngineer Gateway {__version__}")
    parser.add_argument(
        "--config",
        type=str,
        default=None,
    ...
    parser.add_argument(
        ...
        help="Default datasource (default: DATAENGINEER_DATASOURCE env or 'default')",
    )
    ...
    parser.add_argument(
        "--pid-file",
        type=str,
        help=f"PID file path (default: ~/.dataengineer/run/{_SERVICE_NAME}.pid)",
    )
    parser.add_argument(
        "--daemon-log-file",
        type=str,
        help=f"Daemon log file path (default: ~/.dataengineer/logs/{_SERVICE_NAME}.log)",
    )
...
def main() -> None:
    """Main entry point for starting the DataEngineer Gateway."""
    ...
    if args.daemon:
        ...
        log_dir = str(log_file.parent)
        configure_logging(args.debug, log_dir=log_dir, console_output=False)
        logger.info(f"Starting DataEngineer Gateway (daemon) on {args.host}:{args.port} | Debug: {args.debug}")
    ...
    logger.info(f"Starting DataEngineer Gateway on {args.host}:{args.port}")
    _run_gateway(args)
