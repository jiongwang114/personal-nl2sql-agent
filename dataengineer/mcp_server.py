...
DataEngineer's database and context search tools as MCP-compatible tools.
...
Usage:
    ...
    python -m dataengineer.mcp_server --dynamic
    python -m dataengineer.mcp_server --dynamic --host 0.0.0.0 --port 8000

    # Run dynamic server with SSE transport
    python -m dataengineer.mcp_server --dynamic --transport sse
    ...
    uv run dataengineer-mcp --datasource demo
    uv run dataengineer-mcp --datasource demo --transport stdio

    # Run with uvx (after installing from PyPI)
    uvx --from dataengineer-agent dataengineer-mcp --datasource demo
    uvx --from dataengineer-agent dataengineer-mcp --datasource demo --transport stdio

    # Run with HTTP streamable mode (default)
    python -m dataengineer.mcp_server --datasource demo
    python -m dataengineer.mcp_server --datasource demo --host 0.0.0.0 --port 8000

    # Run with HTTP SSE mode
    python -m dataengineer.mcp_server --datasource demo --transport sse --port 8000

    # Run with stdio (for Claude Desktop)
    python -m dataengineer.mcp_server --datasource demo --transport stdio
    ...
    # For Claude Desktop config (claude_desktop_config.json):
    {
        "mcpServers": {
            "dataengineer": {
                "command": "uvx",
                "args": ["--from", "dataengineer-agent", "dataengineer-mcp", "--datasource", "demo", "--transport", "stdio"]
            }
        }
    }
    ...
    # Alternative config using python directly:
    {
        "mcpServers": {
            "dataengineer": {
                "command": "python",
                "args": ["-m", "dataengineer.mcp_server", "--datasource", "demo", "--transport", "stdio"]
            }
        }
    }
...
from dataengineer.utils.mcp_decorators import get_tool_registry  # noqa: E402
...
class LightweightDynamicMCPServer:
    ...
    def __init__(self, config_path: Optional[str] = None, max_cache_size: Optional[int] = 64):
        ...
        self.mcp = FastMCP(
            name="dataengineer",
            instructions=(
                "DataEngineer is a data engineering agent that provides tools for querying databases, "
    ...
    def _register_tools(self):
        ...
        from dataengineer.utils.mcp_decorators import register_dynamic_tools
    ...
    def create_asgi_app(self, transport: Literal["http", "sse"] = "http"):
        ...
        async def lifespan(_app):
            logger.info(f"Starting DataEngineer MCP Server (Lightweight Dynamic Mode, transport={current_transport})")
            ...
            logger.info("DataEngineer MCP Server stopped")
        ...
        async def root(_request):
            ...
            return JSONResponse(
                {
                    "service": "DataEngineer MCP Server",
...
class DataEngineerMCPServer:
    """
    MCP Server that wraps DataEngineer's database and context search tools.
    ...
    def __init__(
        ...
        """
        Initialize the DataEngineer MCP Server.
    ...
    def _register_tools(self):
        ...
        from dataengineer.utils.mcp_decorators import register_static_tools
    ...
    def run(
        ...
        """
        logger.info(f"Starting DataEngineer MCP Server (datasource={self.datasource}, transport={transport})")
    ...
    def _run_http_server(self, app, host: str, port: int, path: str):
        ...
        print(f"\n{'=' * 60}")
        print("  DataEngineer MCP Server (HTTP Mode)")
...
def create_server(
    ...
    """
    Factory function to create a DataEngineerMCPServer instance.
    ...
    Returns:
        Configured DataEngineerMCPServer instance
    """
    return DataEngineerMCPServer(
        datasource=datasource,
        sub_agent=sub_agent,
...
def run_dynamic_server(
    ...
    print(f"\n{'=' * 60}")
    print(f"  DataEngineer MCP Server (Dynamic Mode, transport={transport})")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="DataEngineer MCP Server - Expose Datus tools via Model Context Protocol",
...
Examples:
    ...
    python -m dataengineer.mcp_server --datasource demo --config /path/to/agent.yml
