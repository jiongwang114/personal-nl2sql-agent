...
from dataengineer.agent.evaluate import evaluate_result, setup_node_input
from dataengineer.agent.plan import generate_workflow
...
from dataengineer.utils.traceable_utils import get_trace_url, optional_traceable
...
class WorkflowRunner:
    ...
    def _mark_workflow_failed(self) -> None:
        """Record a terminal failure so saved traces have complete timing data."""

        if self.workflow:
            self.workflow.status = "failed"
            self.workflow.completion_time = time.time()

    def _can_reflect_after_failure(self, node) -> bool:
        """Allow SQL errors to reach ReflectNode for repair decisions."""

        return bool(
            self.workflow
            and node.type == NodeType.TYPE_EXECUTE_SQL
            and self.workflow.get_next_node()
            and self.workflow.get_next_node().type == NodeType.TYPE_REFLECT
        )
    ...
    def run(self, sql_task: Optional[SqlTask] = None, check_storage: bool = False) -> Dict:
        ...
        while self.workflow and not self.workflow.is_complete() and step_count < max_steps:
            ...
            if current_node.status == "failed":
                if self._can_reflect_after_failure(current_node):
                    logger.warning("SQL execution failed; continuing to ReflectNode")
