...
class CompareNode(Node):
    ...
    def _execute_compare(self) -> CompareResult:
        ...
        if not isinstance(self.input, CompareInput):
            raise DataEngineerException(ErrorCode.COMMON_VALIDATION_FAILED, "Input must be a CompareInput instance")

        if not self.model:
            raise DataEngineerException(ErrorCode.COMMON_VALIDATION_FAILED, "Model is not initialized for CompareAgenticNode")
