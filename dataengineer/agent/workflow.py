...
from dataengineer.schemas.node_models import Context, SQLContext, SqlTask
...
class Workflow:
    ...
    def get_last_sqlcontext(self) -> SQLContext:
        ...
        Raises:
            DataEngineerException: If no SQL context is available
        """
        if len(self.context.sql_contexts) > 0:
            return self.context.sql_contexts[-1]
        raise DataEngineerException(ErrorCode.NODE_NO_SQL_CONTEXT)
    ...
    def _init_tools(self):
        ...
        # DBFuncTool expects the configured datasource key, not a physical
        # database name such as a local DuckDB file stem.
        self.tools = db_function_tools(self._global_config, self._global_config.current_datasource)
