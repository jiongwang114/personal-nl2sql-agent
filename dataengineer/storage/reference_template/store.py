...
from dataengineer.storage.base import EmbeddingModel
from dataengineer.storage.subject_tree.store import BaseSubjectEmbeddingStore, base_schema_columns
...
class ReferenceTemplateStorage(BaseSubjectEmbeddingStore):
    ...
    def batch_upsert_templates(self, template_items: List[Dict[str, Any]]) -> None:
        ...
        for item in template_items:
            ...
            if not subject_path:
                raise DataEngineerException(ErrorCode.COMMON_FIELD_REQUIRED, message_args={"field_name": "subject_path"})
