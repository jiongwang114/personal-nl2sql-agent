...
from dataengineer.utils.device_utils import get_device
...
class EmbeddingModel:
    ...
    def model(self):
        ...
        if self.is_model_failed:
            # If the stored error is already a formatted DataEngineerException message, use it directly
            if "error_code=" in self.model_error_message:
                # Extract just the core error message without the DataEngineerException wrapper
                import re
    ...
    def init_model(self):
        ...
        if self.registry_name in (EmbeddingProvider.SENTENCE_TRANSFORMERS, EmbeddingProvider.FASTEMBED):
            ...
            from dataengineer.storage.embedding_openai import OpenAIEmbeddings
