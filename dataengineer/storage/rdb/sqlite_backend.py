...
class SqliteRdbDatabase(RdbDatabase):
    ...
    file under ``{data_dir}/{project}/dataengineer_db/``).
    """
    ...
    def _open_connection(self) -> sqlite3.Connection:
        ...
        try:
            ...
            raise DataEngineerException(ErrorCode.STORAGE_FAILED, message=f"Database connection error: {str(e)}") from e
...
class SqliteRdbBackend(BaseRdbBackend):
    ...
    ``.db`` file at ``{data_dir}/{project}/dataengineer_db/{store_db_name}.db``.
    ...
    def connect(self, project: str, store_db_name: str) -> SqliteRdbDatabase:
        ...
        return SqliteRdbDatabase(os.path.join(self._data_dir, safe_project, "dataengineer_db", f"{safe_store}.db"))
