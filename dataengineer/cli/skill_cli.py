...
from dataengineer.tools.skill_tools.marketplace_auth import clear_token, save_token
...
def run_skill_command(args) -> int:
    ...
    if subcmd == "login":
        ...
        if not skill_args:
            console.print("[red]Usage: dataengineer skill install <name> [version][/]")
            return 1
        name = skill_args[0]
        ...
        if not skill_args:
            console.print("[red]Usage: dataengineer skill publish <path>[/]")
            return 1
        path = skill_args[0]
        ...
        if not skill_args:
            console.print("[red]Usage: dataengineer skill info <name>[/]")
            return 1
        ...
        if not skill_args:
            console.print("[red]Usage: dataengineer skill remove <name>[/]")
            return 1
...
def _cmd_search(manager: SkillManager, query: str):
    if not query:
        console.print("[yellow]Usage: dataengineer skill search <query>[/]")
        return
