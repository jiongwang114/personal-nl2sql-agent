...
:meth:`datus.cli.tui.app.DataEngineerApp.suspend_input` so the outer persistent
...
from dataengineer.cli.cli_styles import print_error, print_success, print_warning
from dataengineer.cli.model_app import ModelApp, ModelSelection, _display_name
...
class ModelCommands:
    ...
    def _run_oauth_flow(self, provider: str) -> bool:
        ...
        if not meta:
            ...
            return False
        from dataengineer.cli.provider_auth_flows import configure_codex_oauth
    ...
    def _persist_custom_model(self, name: str, payload: Dict[str, Any]) -> bool:
        ...
        if not name or not payload:
            ...
            return False
        try:
            from dataengineer.configuration.agent_config import load_model_config
