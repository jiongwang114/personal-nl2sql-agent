...
"""
Interactive initialization command for DataEngineer Agent.
...
from dataengineer.cli.cli_styles import print_error
from dataengineer.cli.init_util import detect_db_connectivity
from dataengineer.cli.provider_auth_flows import configure_claude_subscription, configure_codex_oauth
from dataengineer.configuration.agent_config import AgentConfig
from dataengineer.configuration.project_config import ProjectOverride, ProjectTarget, save_project_override
from dataengineer.utils.loggings import configure_logging, get_logger, print_rich_exception
from dataengineer.utils.path_manager import get_path_manager
from dataengineer.utils.path_utils import safe_rmtree
from dataengineer.utils.resource_utils import copy_data_file, read_data_file_text

logger = get_logger(__name__)


class InteractiveInit:
    """Interactive initialization wizard for DataEngineer Agent."""
    ...
    def __init__(self, user_home: Optional[str] = None):
        ...
        # so we fall back to the context-local path manager / default ~/.dataengineer here.
        ...
        # ``{cwd}/.dataengineer/config.yml`` so each project can pin its own model
        ...
        self.config = {
            "agent": {
                ...
                "storage": {
                    "workspace_root": "~/.dataengineer/workspace",
        ...
        # Staged project-level target; written to ``./.dataengineer/config.yml`` at
    ...
    def run(self) -> int:
        ...
        try:
            self.console.print("\n[bold cyan]Welcome to DataEngineer Init 🎉[/bold cyan]")
    ...
    def _configure_llm(self) -> bool:
        ...
        Result shape (written to ``self.config``):
          ...
          - ``self._pending_target`` — ``ProjectTarget(provider, model)``,
            flushed to ``{cwd}/.dataengineer/config.yml`` in ``_save_configuration``.
    ...
    def _configure_datasource(self) -> bool:
        ...
        from dataengineer.tools.db_tools import connector_registry
    ...
    def _save_configuration(self) -> bool:
        """Persist credentials to ``~/.dataengineer/conf/agent.yml`` and target to
        ``./.dataengineer/config.yml``.
    ...
    def _display_completion(self):
        ...
        self.console.print(f"\nYou are ready to run `dataengineer-cli --datasource {self.datasource_name}` 🚀")
        self.console.print("\nCheck the document at / for more details.")
    ...
    def _test_llm_connectivity(self) -> tuple[bool, str]:
        ...
        try:
            ...
            module = __import__(f"dataengineer.models.{model_type}_model", fromlist=[class_name])
...
class ReferenceSqlStreamHandler:
    ...
    def __init__(self, output_mgr):
        from dataengineer.schemas.batch_events import BatchStage
...
def init_metadata_and_log_result(datasource_name: str, config_path: str, console: Console):
    from dataengineer.configuration.agent_config_loader import load_agent_config
    from dataengineer.storage.schema_metadata.local_init import init_local_schema
    from dataengineer.storage.schema_metadata.store import SchemaWithValueRAG
    ...
    with console.status(f"→ Initializing metadata for {datasource_name} with path `{storage_path}`..."):
        try:
            if kb_update_strategy == "overwrite":
                ...
                from dataengineer.storage.backend_holder import create_vector_connection
...
def do_init_sql_and_log_result(
    ...
    from dataengineer.storage.reference_sql.reference_sql_init import init_reference_sql
    from dataengineer.storage.reference_sql.store import ReferenceSqlRAG
    from dataengineer.utils.stream_output import StreamOutputManager
