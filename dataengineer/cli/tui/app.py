...
The DataEngineer REPL historically used ``PromptSession.prompt()`` and re-rendered
the status bar as a prefix each round. During agent loops, the session was
not active and the status bar + input disappeared. :class:`DataEngineerApp` replaces
...
EXIT_SENTINEL = "__dataengineer_tui_exit__"
...
def tui_enabled() -> bool:
    ...
    ``DATAENGINEER_TUI`` environment variable is not set to a falsy value
    (``0``/``false``/``no``/``off``, case-insensitive).
    """
    env = os.environ.get("DATAENGINEER_TUI", "").strip().lower()
...
class DataEngineerApp:
    ...
    def __init__(
        ...
        *,
        ...
        self._executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="dataengineer-tui-worker")
        ...
        # ``completion-menu.*`` keys in ``DataEngineerCLI._build_app_style``.
    ...
    def suspend_input(self, ready_timeout: float = 2.0) -> Iterator[None]:
        ...
        if not released.wait(timeout=ready_timeout):
            ...
            try:
                ...
                pass
            raise RuntimeError("DataEngineerApp failed to release stdin within timeout")
    ...
    def run(self) -> int:
        ...
        caller *before* printing the banner (see ``DataEngineerCLI._pin_to_bottom``),
    ...
    def _build_default_key_bindings(self) -> KeyBindings:
        ...
        :attr:`key_bindings` so DataEngineerCLI can keep its existing handlers close
