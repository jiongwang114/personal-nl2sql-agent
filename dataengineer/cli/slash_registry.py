...
* :attr:`DataEngineerCLI.commands` — maps ``/name`` (and aliases) to the handler bound
...
* :meth:`DataEngineerCLI._cmd_help` — emits the grouped help text.

Adding a command: append a :class:`SlashSpec` below, then wire the matching
handler in ``DataEngineerCLI._build_slash_handler_map`` so the registry integrity
