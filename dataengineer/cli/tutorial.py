...
from dataengineer.cli.interactive_init import parse_subject_tree
...
from dataengineer.utils.loggings import get_logger, print_rich_exception
...
class BenchmarkTutorial:
    ...
    def _ensure_files(self):
        ...
        from dataengineer.cli.interactive_init import copy_data_file
    ...
    def _ensure_config(self) -> bool:
        if self.config_path and not Path(self.config_path).expanduser().resolve().exists():
            self.console.print(
                ...
                "please check it or run `dataengineer-agent init` first."
            )
            return False
        ...
        if (
            ...
            services_config["datasources"]["california_schools"] = {
                ...
                "uri": "~/.dataengineer/benchmark/california_schools/california_schools.sqlite",
            }
    ...
    def run(self):
        try:
            self.console.print("[bold cyan]Welcome to DataEngineer benchmark data preparation tutorial 🎉[/bold cyan]")
            self.console.print(
            ...
            from dataengineer.cli.interactive_init import init_metadata_and_log_result, overwrite_sql_and_log_result

            self.console.print("[bold yellow][2/6] Initialize Metadata using command: [/bold yellow]")
            self.console.print(
                f"    [bold green]dataengineer-agent[/] [bold]bootstrap-kb --config {self.config_path} "
            ...
            with self.console.status("Sub-Agents Building...") as status:
                ...
                self.console.print(
                    ...
                    "`dataengineer-agent benchmark --workflow datus_schools`.[/]"
                )
