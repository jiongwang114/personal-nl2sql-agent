...
from dataengineer.utils.async_utils import setup_windows_policy
...
class ArgumentParser:
    ...
    def _setup_arguments(self):
        ...
        self.parser.add_argument("-v", "--version", action="version", version=f"DataEngineer CLI {__version__}")
        ...
        self.parser.add_argument(
            ...
            type=str,
            default=None,
            help="Path to @dataengineer/web-chatbot dist directory (for --web mode)",
        )
...
class Application:
    ...
    def run(self):
        ...
        # REPL-only: ensure ./.dataengineer/config.yml exists before anything touches
        ...
        if args.print_mode is not None:
            from dataengineer.cli.print_mode import PrintModeRunner

            PrintModeRunner(args).run()
        elif args.web:
            self._run_web_interface(args)
        else:
            cli = DataEngineerCLI(args)
            cli.run()
    ...
    def _ensure_project_config(self, args) -> None:
        """Ensure ``./.dataengineer/config.yml`` exists; create a minimal one when absent.
                ...
                raise DataEngineerException(
                    ...
                    message_args={
                        "config_error": (
                            ...
                            f"default_datasource={override.default_datasource!r} in .dataengineer/config.yml."
                        )
                    },
                )
                ...
                raise DataEngineerException(
                    ...
                    message_args={
                        "config_error": (
                            ...
                            f"Edit .dataengineer/config.yml manually or rerun in an interactive terminal."
                        )
                    },
                )
            console.print(
    ...
    def _provider_has_credentials(provider: str, raw: dict) -> bool:
        ...
        from dataengineer.configuration.agent_config import _load_provider_catalog, resolve_env
    ...
    def _run_web_interface(self, args):
        ...
        from dataengineer.cli.web import run_web_interface
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "skill":
        from dataengineer.main import create_parser as create_main_parser
