...
def _build_agent_args(args: argparse.Namespace) -> argparse.Namespace:
    """Bridge CLI ``data_engineer web`` arguments to the shape expected by
    ``create_app`` / ``DataEngineerAPIService``.
    """
    agent_args = argparse.Namespace(
        datasource=args.datasource,
        config=getattr(args, "config", None),
        debug=getattr(args, "debug", False),
        # Fields expected by DataEngineerAPIService but not present in CLI args
        max_steps=20,
...
def create_web_app(args: argparse.Namespace) -> FastAPI:
    ...
    ----------
    args:
        Parsed CLI arguments from ``data_engineer web``.
    """
...
def run_web_interface(args: argparse.Namespace) -> None:
    """Entry point called by ``data_engineer web``.

    Creates the FastAPI app and starts uvicorn.
    """
    from dataengineer.cli.web.config_manager import get_home_from_config
    ...
    logger.info("Starting DataEngineer Web Interface...")
