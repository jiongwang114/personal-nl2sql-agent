...
def setup_tracing():
    """Set up LangSmith tracing with DataEngineerTracingProcessor.

    Creates a DataEngineerTracingProcessor (subclass of OpenAIAgentsTracingProcessor)
    ...
    try:
        ...
        class DataEngineerTracingProcessor(OpenAIAgentsTracingProcessor):
        ...
        _tracing_processor = DataEngineerTracingProcessor()
        set_trace_processors([_tracing_processor])
        logger.info("LangSmith DataEngineerTracingProcessor enabled for SDK tracing")
    except ImportError:
