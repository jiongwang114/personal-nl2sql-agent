...
"""
Centralized path management for all .dataengineer related directories and files.

This module provides a unified interface for managing all paths related to the
.dataengineer directory structure. The home directory is determined from agent.yml config.
...
- ``{project_root}/.dataengineer/skills/`` — project-level skills.
...
- ``{datus_home}/data/`` — storage-backend root.  Each backend owns its own
  ...
  document/ storage) should use :pyattr:`DataEngineerPathManager.project_data_dir`.
...
# callers, so we re-check to keep ``~/.dataengineer/data/{project_name}`` and
# ``~/.dataengineer/sessions/{project_name}`` safely sandboxed.
_PROJECT_NAME_SEGMENT_RE = re.compile(r"^[A-Za-z0-9_.\-]+$")


class DataEngineerPathManager:
    """
    Centralized manager for all .dataengineer related paths.

    The home directory can be customized via agent.yml config (agent.home).
    If not configured, defaults to ~/.dataengineer

    Example:
        >>> from dataengineer.utils.path_manager import get_path_manager
        >>> pm = get_path_manager()
    ...
    def __init__(
        ...
        Args:
            datus_home: Custom .dataengineer root directory. If None, defaults to ~/.dataengineer
            project_name: Logical project identifier used to shard ``sessions/`` and
                ...
                ``sessions_dir`` without a project_name raises ``DataEngineerException``.
            project_root: Root directory for project-scoped KB content (the
                ``subject/`` tree and ``.dataengineer/skills``). Defaults to ``Path.cwd()``.
        """
    ...
    def _validate_project_name_segment(project_name: Optional[str]) -> str:
        ...
        if not _PROJECT_NAME_SEGMENT_RE.match(project_name):
            raise ValueError(
                ...
                f"_normalize_project_name before constructing DataEngineerPathManager."
            )
        return project_name

    @staticmethod
    def resolve_home(datus_home: Optional[PathLike] = None) -> Path:
        """Resolve a configured home path or fall back to ``~/.dataengineer``."""
        if datus_home:
    ...
    def update_home(self, new_home: PathLike) -> None:
        """
        Update the dataengineer home directory.

        Deprecated compatibility helper.
        Prefer creating a new ``DataEngineerPathManager(new_home, ...)`` and passing it
        explicitly.
        """
        import warnings

        warnings.warn(
            "DataEngineerPathManager.update_home is deprecated; construct a new DataEngineerPathManager instead",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def datus_home(self) -> Path:
        """Root .dataengineer directory path"""
    ...
    def conf_dir(self) -> Path:
        """Configuration directory: ~/.dataengineer/conf"""
        return self._datus_home / "conf"

    @property
    def data_dir(self) -> Path:
        """Storage-backend root: ``~/.dataengineer/data``.
    ...
    def project_data_dir(self) -> Path:
        """Project-scoped local data directory: ``~/.dataengineer/data/{project_name}``.

        Requires ``project_name`` to be set; raises ``DataEngineerException`` when it
    ...
    def logs_dir(self) -> Path:
        """Logs directory: ~/.dataengineer/logs"""
        return self._datus_home / "logs"

    @property
    def sessions_dir(self) -> Path:
        """Sessions directory: ``~/.dataengineer/sessions/{project_name}``.
    ...
    def template_dir(self) -> Path:
        """Template directory: ~/.dataengineer/template"""
        return self._datus_home / "template"

    @property
    def sample_dir(self) -> Path:
        """Sample directory: ~/.dataengineer/sample"""
        return self._datus_home / "sample"

    @property
    def run_dir(self) -> Path:
        """Runtime directory: ~/.dataengineer/run"""
        return self._datus_home / "run"

    @property
    def benchmark_dir(self) -> Path:
        """Benchmark directory: ~/.dataengineer/benchmark"""
        return self._datus_home / "benchmark"

    @property
    def save_dir(self) -> Path:
        """Save directory: ~/.dataengineer/save"""
        return self._datus_home / "save"

    @property
    def workspace_dir(self) -> Path:
        """Workspace directory: ~/.dataengineer/workspace"""
        return self._datus_home / "workspace"

    @property
    def trajectory_dir(self) -> Path:
        """Trajectory directory: ~/.dataengineer/trajectory"""
    ...
    def project_skills_dir(self) -> Path:
        """Project-level skills directory: ``{project_root}/.dataengineer/skills``."""
    ...
    def agent_config_path(self) -> Path:
        """Agent configuration file: ~/.dataengineer/conf/agent.yml"""
        return self.conf_dir / "agent.yml"

    def mcp_config_path(self) -> Path:
        """MCP configuration file: ~/.dataengineer/conf/.mcp.json"""
        return self.conf_dir / ".mcp.json"

    def auth_config_path(self) -> Path:
        """Authentication configuration file: ~/.dataengineer/conf/auth_clients.yml"""
        return self.conf_dir / "auth_clients.yml"

    def history_file_path(self) -> Path:
        """Command history file: ~/.dataengineer/history"""
        return self._datus_home / "history"

    def dashboard_path(self) -> Path:
        return self._datus_home / "dashboard"

    def pid_file_path(self, service_name: str = "dataengineer-agent-api") -> Path:
        """
        ...
        Returns:
            Path to PID file: ~/.dataengineer/run/{service_name}.pid
        """
    ...
    def rag_storage_path(self) -> Path:
        ...
        Returns:
            Path: ``{project_data_dir}/dataengineer_db`` (i.e. ``{home}/data/{project_name}/dataengineer_db``)
        """
        path = self.project_data_dir / "dataengineer_db"
    ...
    def resolve_config_path(self, filename: str, local_path: Optional[str] = None) -> Path:
        ...
        # 3. Default to ~/.dataengineer/conf
    ...
    def ensure_templates(self):
        ...
        from dataengineer.utils.resource_utils import copy_data_file
...
# We store the full ``DataEngineerPathManager`` instance (not just the home path string) so that
# ``project_name`` / ``project_root`` and any other instance-level state survives the
# ContextVar round-trip.
_current_path_manager: ContextVar[Optional["DataEngineerPathManager"]] = ContextVar("datus_current_path_manager", default=None)


def set_current_path_manager(
    path_manager: Optional[Union["DataEngineerPathManager", PathLike]] = None,
    *,
    agent_config: Optional[Any] = None,
) -> Token:
    """Set the current context-local path manager used by ``get_path_manager()``.

    The full ``DataEngineerPathManager`` instance is stored, so ``project_name`` /
    ...
    if isinstance(path_manager, DataEngineerPathManager):
        resolved: Optional["DataEngineerPathManager"] = path_manager
    elif path_manager:
        resolved = DataEngineerPathManager(path_manager)
    else:
        resolved = None
...
def get_path_manager(
    datus_home: Optional[PathLike] = None,
    *,
    path_manager: Optional["DataEngineerPathManager"] = None,
    agent_config: Optional[Any] = None,
    ...
    4. Context-local ``DataEngineerPathManager`` set via ``set_current_path_manager()``
    5. ``~/.dataengineer``

    Args:
        datus_home: Optional custom .dataengineer root directory.
        path_manager: Optional explicit path manager instance to reuse.
        agent_config: Optional config object exposing ``path_manager``.

    Returns:
        DataEngineerPathManager instance
    """
    ...
    if datus_home is not None:
        return DataEngineerPathManager(datus_home)

    current = _current_path_manager.get()
    if current is not None:
        return current
    return DataEngineerPathManager()
