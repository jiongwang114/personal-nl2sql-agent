...
def _is_source_environment() -> bool:
    ...
    try:
        ...
        # Navigate up to project root (from dataengineer/utils/ to project root)
        project_root = os.path.dirname(os.path.dirname(current_dir))

        # Check for source code markers: pyproject.toml and dataengineer/ directory
        has_pyproject = os.path.exists(os.path.join(project_root, "pyproject.toml"))
        has_dataengineer_dir = os.path.exists(os.path.join(project_root, "dataengineer"))

        return has_pyproject and has_dataengineer_dir
    except Exception:
        return False


def _default_log_dir(
    *, path_manager: Optional["DataEngineerPathManager"] = None, agent_config: Optional[Any] = None
) -> str:
    """Resolve a writable default log directory for source and packaged runs."""
    ...
    return str(get_path_manager(path_manager=path_manager, agent_config=agent_config).logs_dir)
...
class DynamicLogManager:
    ...
    def __init__(self, debug=False, log_dir=None, path_manager=None, agent_config=None):
        self.debug = debug
        # Default to ~/.dataengineer/logs (via path manager) when log_dir is not specified.
        if log_dir is None:
            log_dir = _default_log_dir(path_manager=path_manager, agent_config=agent_config)
...
def configure_logging(
    ...
    *,
    ...
    Args:
        ...
        log_dir: Directory for log files. If None, defaults to ``~/.dataengineer/logs``
                 (resolved via the active ``DataEngineerPathManager``).
